...
#pragma mark - 常见运动轨迹插值
    inline vector<Scribble::BrushPoint> interpolate(Point pStartPosition, Point pEndPosition, float pDelta);//两点间线性插值
    inline vector<Scribble::BrushPoint> interpolate(Point start, Point end, float stepMin, float stepMax);
...
#pragma mark - paint
     ...
     */
    virtual void paint(Sprite * lTarget, Point &pPosition);
    ...
    void setStepMax(float stepMax);
    void setStepMin(float stepMin);
