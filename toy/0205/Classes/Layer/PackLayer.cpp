...
bool PackLayer::init(){
    ...
    if(Layer::init())
    {
        ...
        auto pos = Vec2(Director::getInstance()->getWinSize()/2) + Vec2(4, 28);
        ArmatureDataManager::getInstance()->addArmatureFileInfo("Choose_1.ExportJson");
        
        auto sun = quickGetAnimation(0, "Choose_1", pos);
        _root->addChild(sun);
...
void PackLayer::reload(){
    if(!xIAP->isPurchased(iap_1)){
        auto purchase = Sprite::create("choose-1/IAP-1.png");
...
void PackLayer::initAniamtion()
{
    ...
    Vec2 pos = Vec2(_list->getWorldPosition().x /2 + 20, _list->getWorldPosition().y + 15);
    _model->setPosition(pos);
    _model->setLocalZOrder(1);
    
    _model->add("ModelWait1.ExportJson", this);
    auto armature = _model->add("ModelWait2.ExportJson", this, Vec2(-22, 71));
