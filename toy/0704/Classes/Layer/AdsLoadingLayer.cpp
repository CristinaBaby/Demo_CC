...
void AdsLoadingLayer::loadToyboxStudio()
{
    ...
    bear->setPosition(Vec2(getContentSize().width/2, getContentSize().height/2 + 80));
    addChild(bear);
    
    //
    auto loading = Sprite::create(RES_TOYBOX_LOADING);
    loading->setPosition(Vec2(getContentSize().width/2 - 10, getContentSize().height/2 - 60));
    addChild(loading);
    ...
    addChild(dot1);
    dot1->setPosition(Vec2(loading->getPosition().x + loading->getContentSize().width/2 + 50, loading->getPosition().y + 0));

    auto dot2 = Sprite::create(RES_TOYBOX_DOT);
    addChild(dot2);
    dot2->setPosition(Vec2(loading->getPosition().x + loading->getContentSize().width/2 + 30, loading->getPosition().y + 0));
    
    auto dot3 = Sprite::create(RES_TOYBOX_DOT);
    addChild(dot3);
    dot3->setPosition(Vec2(loading->getPosition().x + loading->getContentSize().width/2 + 10, loading->getPosition().y + 0));
    dot3->runAction(RepeatForever::create(Sequence::create(CallFunc::create([=]{
        dot2->setVisible(true), dot1->setVisible(false);
    }), DelayTime::create(ANIMATION_DURATION), CallFunc::create([=]{
        dot1->setVisible(true);
    }), DelayTime::create(ANIMATION_DURATION), CallFunc::create([=]{
        dot2->setVisible(false), dot1->setVisible(false);
...
void AdsLoadingLayerEx::loadToyboxStudio()
{
    //bg
    auto bg = Sprite::create(RES_TOYBOX_BG);
    bg->setPosition(Vec2(getContentSize().width/2, getContentSize().height/2));
