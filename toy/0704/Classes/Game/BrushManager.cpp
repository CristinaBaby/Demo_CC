...
void BrushManager::init(FillNode *pictureScribble)
{
...
BrushManager::BrushManager()
{
    ...
    ValueMap colorVector = FileUtils::getInstance()->getValueMapFromFile("colors.plist");
    ...
    ValueVector items = colorVector["Pens"].asValueVector();
    for (auto itor = items.begin(); itor != items.end(); itor++)
    {
        auto dic = itor->asValueMap();
        
        if (_colors.empty()) {
            _colors.push_back(Color4F::WHITE);
        }
        else
        {
            Color4F color;
            color.r = dic["R"].asFloat()/255;
            color.g = dic["G"].asFloat()/255;
            color.b = dic["B"].asFloat()/255;
            color.a = dic["A"].asFloat()/255;
...
void BrushManager::setColor(int colorsIndex)
{
    ...
    else
    {
        ...
        useColor(xBrush->getColors().at(colorsIndex));
    }
}
...
void BrushManager::useColor(const Color4F &color)
{
    _pictureScribble->useTarget(color);
    _pictureScribble->getColoringNode()->setStartColor(Color4B(color));
}
