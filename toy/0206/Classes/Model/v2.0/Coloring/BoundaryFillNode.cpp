...
//    CCLOG("getPixelColor %d %d %d %d at %d %d", r, g, b, a , pX, pY);
    ...
    int index = (pY * (int)getSize().width + pX);
    if(index > _dataSize || index < 0)
    {
...
//    lPass = !(lIsBigGray/* || lIsBlack*/);
    if (abs(pColor.r - pBoundaryColor.r) < _boundaryTolerance &&
        abs(pColor.g - pBoundaryColor.g) < _boundaryTolerance &&
        abs(pColor.b - pBoundaryColor.b) < _boundaryTolerance &&
        abs(pColor.a - pBoundaryColor.a) < _boundaryTolerance) {
        lPass = false;
    }
    else{
        lPass = true;
...
bool BoundaryFillNode::init(string pLineFrameFile){
    ...
    _dataSize = lLineImage->getDataLen() / 4;
