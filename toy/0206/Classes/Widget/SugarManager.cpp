...
void SugarManager::update(float dt){
    ...
    float limit = 0.05 + quickRandomFloat(0, 0.07);
    if (_acc > limit) {
        int index = quickRandom(1, 3);
        
        string name1 = "sugar" + std::to_string(index) + "_1" + ".png";
        auto sugar = Sprite::create("mm/" + name1);
        
        string name2 = "sugar" + std::to_string(index) + "_2" + ".png";
        auto part2 = Sprite::create("mm/" + name2);
        
        part2->setColor(Color3B(PensColorManager::getColorManager()->getSugarColor()));
        part2->setPosition(sugar->getContentSize()/2);
        sugar->addChild(part2);
        sugar->setScale(0.5f);
