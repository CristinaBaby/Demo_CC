...
string CSVParse::getData(unsigned int rows,unsigned int cols)
{
    if (rows>=data.size()|| cols>=data[rows].size()) {
        return "";
    }
    
    string ret = data[rows][cols];
...
string CSVParse::getDatas( unsigned int rows,const char* colsName )
{
    return std::move(getData(rows, findRowsData(0 , colsName)));
}
