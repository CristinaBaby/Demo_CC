...
void AudioHelper::changeMusicVolume(float volume)
{
    ...
    _data->setFloatForKey(key_music_volume, mMusicVolume);
    _data->flush();
}
...
void AudioHelper::changeSoundVolume(float volume)
{
    ...
    _data->setFloatForKey(key_sound_volume, mSoundVolume);
    _data->flush();
}
...
{
    // load volume
    mMusicVolume = _data->getFloatForKey(key_music_volume, DEFAULT_VOLUME);
    mSoundVolume = _data->getFloatForKey(key_sound_volume, DEFAULT_VOLUME);
