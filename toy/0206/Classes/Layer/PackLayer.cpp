...
bool PackLayer::init(){
    ...
    if(Layer::init())
    {
        ...
        _back = quickButton("back-1 ", _root, CC_CALLBACK_2(PackLayer::onButton, this));
        _restore = quickButton("Restore Purchase", _root, CC_CALLBACK_2(PackLayer::onButton, this));
        ...
        _list->setContentSize(_list->getContentSize());
        ...
        auto pos = Vec2(Director::getInstance()->getWinSize()/2) + Vec2(4, -23);
        ArmatureDataManager::getInstance()->addArmatureFileInfo("Choose_1.ExportJson");
        
        auto backGround = quickGetAnimation(0, "Choose_1", pos);
        _root->addChild(backGround,10);
        
//        auto posBook3 = book3->getPosition() + Vec2(-270, 150);
//        auto butterfly1 = quickGetAnimation(1, "Choose_1", posBook3); //fly
//        book3->addChild(butterfly1, 2);
        
//        auto posBook2 = book2->getPosition() + Vec2(-50, 180);
//        auto butterfly2 = quickGetAnimation(2, "Choose_1", posBook2);//stop
//        book2->addChild(butterfly2, 2);
        
        //////////////////////////////////////////////////////////// add next scene resource ////////////////////////////////////////////////////////////
        xAsync->add("choose-2.json");
        ArmatureDataManager::getInstance()->removeArmatureFileInfo("Home.ExportJson");
        xAsync->add("Choose_2.ExportJson");
        //////////
        reload();
...
void PackLayer::reload(){
    if(!xIAP->isPurchased(iap_1)){
        auto purchase = Sprite::create("choose-1/IAP.png");
...
void PackLayer::onButton(Ref* pSender, Widget::TouchEventType type)
{
    ...
    if (name == "back-1 " || name == "Restore Purchase") {
    ...
    else if(name.compare("Restore Purchase") == 0){
        xIAP->restore();
    }
    else
    {
...
void PackLayer::initAniamtion()
{
    ...
    Vec2 pos = Vec2(_list->getWorldPosition().x /2 + 10, _list->getWorldPosition().y + 60);
    _model->setPosition(pos);
    _model->setLocalZOrder(1);
    
    _model->add("ModelWait1.ExportJson", this);
    auto armature = _model->add("ModelWait2.ExportJson", this, Vec2(-10,55));
...
void PackLayer::onMovementEvent(cocostudio::Armature *armature, cocostudio::MovementEventType movementType){
    if (MovementEventType::LOOP_COMPLETE == movementType || MovementEventType::COMPLETE == movementType) {
        _model->switchArmature();
    }
}
