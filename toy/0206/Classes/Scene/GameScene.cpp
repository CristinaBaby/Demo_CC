...
bool GameScene::init(int packIndex, int pictureIndex){
    ...
    if(Scene::init())
    {
        ...
        //////////////////////////////////////////////////////////// UI ////////////////////////////////////////////////////////////
        Size size(772, 690);
        auto layerWhite = LayerColor::create(Color4B::WHITE, size.width, Director::getInstance()->getWinSize().height);
        layerWhite->setPosition(Utils::getInstance()->getScreenCenter().x + (1136 - size.width)/2 - layerWhite->getContentSize().width/2,
                                      Utils::getInstance()->getScreenCenter().y + (768 - size.height)/2 - layerWhite->getContentSize().height/2);
        addChild(layerWhite);
        ...
        _root->addChild(cilper, 3);
        cilper->addChild(_cliperLayer,100);
        
        ArmatureDataManager::getInstance()->addArmatureFileInfo("MM.ExportJson");
        auto bgAnimation = quickGetAnimation(0, "MM", Vec2(405, -35));
        _cliperLayer->addChild(bgAnimation, 1);
...
void GameScene::onProgress(int percent)
{
    switch (percent) {
        ...
        case 2: //////////////////////////////////////////////////////////// UI ////////////////////////////////////////////////////////////
        {
            ...
            auto stupid = (ImageView*)Helper::seekWidgetByName(_root, "grass-4");
            stupid->setTouchEnabled(true);
...
{
    SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Model0.plist");
    SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Model1.plist");
    
    auto zero = _center->convertToNodeSpace(Vec2::ZERO);
    Vec2 pos((zero.x + (_backBtn->getPosition().x))/2, _backBtn->getPosition().y + 120);
    
    Vec2 modelPos = _center->convertToWorldSpace(pos);
    modelPos = _root->convertToNodeSpace(modelPos);
    
    _model = make_shared<ArmatureManager>();
    _model->setScale(0.65);
    _model->setPosition(modelPos);
    _model->setLocalZOrder(5);
    
    _model->add("ModelWait1.ExportJson", _root);
    _model->add("ModelWait3.ExportJson", _root);
    _model->add("Paint.ExportJson", _root, Vec2(13, -16));
    
    auto armature = _model->add("ModelWait2.ExportJson", _root, Vec2(-8, 50));
    ...
    armature = _model->add("Talk.ExportJson", _root);
    armature->getAnimation()->setFrameEventCallFunc(CC_CALLBACK_0(GameScene::onFrameEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    armature = _model->add("Suggest.ExportJson", _root, Vec2(12, -2));
    armature->getAnimation()->setFrameEventCallFunc(CC_CALLBACK_0(GameScene::onFrameEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
