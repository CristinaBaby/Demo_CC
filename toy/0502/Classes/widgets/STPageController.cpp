...
{
    // stop the bounce animation which set in super class ScrollView
...
{
    CCASSERT( (std::is_convertible<STPage*, TableViewCell*>::value), "Invalid Cell Type for STPageController!");

    _mpPageCell = dynamic_cast<STPage *>(cell);
}
...
static const float DEFAULT_MARGIN = MultiScreenHelper::getValue(10);
