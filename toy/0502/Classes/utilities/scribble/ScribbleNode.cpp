...
bool ScribbleNode::useBrush(string pBrushFile, Scribble::BrushType pBrushType/* = Scribble::eBrush*/, float pScale/* = 1.0*/){
    ...
    if(fabs(pScale - 1.0) <= 0.001){
        ...
        RenderTextureEx *lRenderTexture = RenderTextureEx::create(lBrushSprite->getContentSize().width * pScale, lBrushSprite->getContentSize().height * pScale, Texture2D::PixelFormat::RGBA8888, GL_DEPTH24_STENCIL8);
