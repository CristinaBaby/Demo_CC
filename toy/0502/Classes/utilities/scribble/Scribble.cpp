...
void Scribble::antiAliasing(RenderTextureEx *pRenderTexture){
    pRenderTexture->getSprite()->getTexture()->setAntiAliasTexParameters();
}

void Scribble::disAntiAliasing(RenderTextureEx *pRenderTexture){
...
void Scribble::setTargetSolid(bool pIsSolid, float pSolidRate){
    ...
    if (pIsSolid) {
        ...
        BlendFunc f = {GL_ZERO,GL_SRC_ALPHA };//GL_DST_ALPHA  GL_ONE
        lBrush->setBlendFunc(f);
    }
...
void Scribble::paint(Sprite *pTarget, RenderTextureEx *pCanvas, Point pPosition){
    vector<Point> lPositions;
    lPositions.push_back(pPosition);
    this->paint(pTarget, pCanvas, lPositions);
}

void Scribble::paint(Sprite *pTarget, RenderTextureEx *pCanvas, Point pStartPosition, Point pEndPosition){
    Sprite *lBrush = this->getBrush();
...
void Scribble::paint(Sprite *pTarget, RenderTextureEx *pCanvas, vector<Point> pWorldPositions){
    Sprite *lBrush = this->getBrush();
...
void Scribble::removeTargetObserver(RenderTextureEx *pTarget){
