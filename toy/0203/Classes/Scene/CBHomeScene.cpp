...
#define RES_BUTTON_POLICY       "home_btn_privacy.png"
...
bool HomeScene::init()
{
    ...
    do {
        ...
        CC_BREAK_IF(!logo);
        logo->setPosition(Vec2(Utils::SCREEN_WIDTH/2, Utils::SCREEN_HEIGHT*2/3));
        addChild(logo);
        ...
        touchEventListener->onTouchEnded = [play, setting, more, restore, policy, this](Touch* touch, Event* event) {
        ...
        touchEventListener->onTouchMoved = [play, setting, more, restore, policy](Touch* touch, Event* event) {
        ...
        touchEventListener->onTouchCancelled = [play, setting, more, restore, policy](Touch* touch, Event* event) {
...
void HomeScene::playAction()
{
    ...
    bool shouldShowAds = UserDefault::getInstance()->getBoolForKey("shouldShowCross", false);
    if(shouldShowAds)
    {
    ...
    UserDefault::getInstance()->setBoolForKey("shouldShowCross", true);
...
{
    Layer::onExit();
     KDKeyboardDispatcher::getInstance()->removeDelegate(this);
}
void HomeScene::onEnter()
{
     Layer::onEnter();
    
    KDKeyboardDispatcher::getInstance()->addDelegate(this);
}
