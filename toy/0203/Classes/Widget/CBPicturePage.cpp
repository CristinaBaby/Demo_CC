...
void PicturePage::reload(const int picturePageIndex)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        for (int j = 0; j < 3; ++j)
        {
            ...
            //white background
            auto bg = Sprite::create("ui03_picture.png");
            ...
            logo->setPosition(Vec2(offsetX + logo->getContentSize().width/2, getContentSize().height - offsetY - logo->getContentSize().height/2));
...
//            Director::getInstance()->getTextureCache()->addImageAsync(logoName, [=](Texture2D *texture){
//                
//                float offsetX = 0;
//                float offsetY = 0;
//
