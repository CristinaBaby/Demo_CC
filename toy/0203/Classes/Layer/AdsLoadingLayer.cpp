...
void AdsLoadingLayer::onAdsLoaded(ADS_TYPE adType)
{
    AdLoadingLayerBase::onAdsLoaded(adType);
        auto pictureLayer = dynamic_cast<colorbook::PictureScene*>(cocos2d::Director::getInstance()->getRunningScene()->getChildren().at(0));
        if (pictureLayer)
        {
        ...
        auto packLayer = dynamic_cast<colorbook::PackScene*>(cocos2d::Director::getInstance()->getRunningScene()->getChildren().at(0));
        if (packLayer)
        {
...
bool AdsLoadingLayer::onKeyReleased()
{
    return true;
}
