...
vector<Scribble::BrushPoint> ScribbleNode::interpolate(Point pStartPosition, Point pEndPosition, float pDelta){
    return getScribble()->interpolate(pStartPosition, pEndPosition, pDelta);
}

vector<Scribble::BrushPoint> ScribbleNode::interpolate(Point start, Point end, float stepMin, float stepMax){
    return getScribble()->interpolate(start, end, stepMin, stepMax);
}
...
void ScribbleNode::paint(Sprite * lTarget, Point &pPosition){
    
    Point lPosition = _canvas->getSprite()->convertToNodeSpace(pPosition);
    lTarget->setPosition(lPosition);
    Size lCanvasSize = _canvas->getSprite()->getContentSize();
...
void ScribbleNode::setDensity(float density){
    _scribble->setDensity(density);
}

void ScribbleNode::setOpacity(float opacity){
    _scribble->setOpacity(opacity);
}

void ScribbleNode::setSolid(bool solid)
{
    _scribble->setSolid(solid);
}

void ScribbleNode::setStepMax(float stepMax)
{
    _scribble->setStepMax(stepMax);
}

void ScribbleNode::setStepMin(float stepMin)
{
    _scribble->setStepMin(stepMin);
}
