...
vector<Scribble::BrushPoint> Scribble::interpolate(Point pStartPosition, Point pEndPosition, float pDelta){
    float lDistance = pStartPosition.getDistance(pEndPosition);
    int lDis = (int)lDistance;
    
    vector<BrushPoint> lPositions;
    for (int i = 0; i < lDis; i += pDelta) {
        ...
        lPositions.push_back(BrushPoint(lPosition));
    }
    return lPositions;
}

vector<Scribble::BrushPoint> Scribble::interpolate(Point start, Point end, float stepMin, float stepMax){
    float distance = start.getDistance(end);
    float distanceX = end.x - start.x;
    float distanceY = end.y - start.y;
    float distanceHalf = distance / 2;
    
    vector<BrushPoint> points;
    //前半部分, step,从stepMin, 逐渐增加到stepMax
    float step = stepMin;
    
    float distanceCur = 0;
    for (; distanceCur < distanceHalf; distanceCur += step) {
        float percentage = float(distanceCur) / distance;
        step+=0.25;
        if (step > stepMax) {
            step = stepMax;
        }
        
        Point lPosition(start.x + (distanceX * percentage), start.y + (distanceY * percentage));
        BrushPoint brush;
        brush.pos = lPosition;
        points.push_back(brush);
    }
    
    for (; distanceCur < distance; distanceCur += step) {
        float percentage = float(distanceCur) / distance;
        step-=0.25;
        if (step < stepMin) {
            step = stepMin;
...
void Scribble::paint(Sprite *pTarget, RenderTexture *pCanvas, Point pPosition){
    vector<BrushPoint> lPositions;
    lPositions.push_back(BrushPoint(pPosition));
    paint(pTarget, pCanvas, lPositions);
}

void Scribble::paint(Sprite *pTarget, RenderTexture *pCanvas, Point pStartPosition, Point pEndPosition){
    Sprite *lBrush = this->getBrush();
    int lDelta = lBrush->getContentSize().width / _density * lBrush->getScaleX();//5.0 * pBrush->getScale();  5.0 * pBrush->getScale();//
    vector<BrushPoint> lPositions;
    
    if (_solid) {
        lPositions = interpolate(pStartPosition, pEndPosition, lDelta);
    }
    else
    {
        lPositions = interpolate(pStartPosition, pEndPosition, _stepMin, _stepMax);
    }
    
    paint(pTarget, pCanvas, lPositions);
}

void Scribble::paint(Sprite *pTarget, RenderTexture *pCanvas, vector<BrushPoint> pWorldPositions){
    Sprite *lBrush = this->getBrush();
    
    for (int i = 0; i < pWorldPositions.size(); ++i) {
        Point lPosition = pCanvas->getSprite()->convertToNodeSpace(pWorldPositions.at(i).pos);
...
void Scribble::setOpacity(float opacity)
{
    Sprite *lBrush = this->getBrush();
    lBrush->setOpacity(opacity);
}