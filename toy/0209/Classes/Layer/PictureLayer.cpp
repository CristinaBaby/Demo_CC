...
bool PictureLayer::init(const int index, const int count){
    ...
    if(Layer::init())
    {
        ...
        _backBtn = quickButton("back", _root, CC_CALLBACK_2(PictureLayer::onButton, this));
        ...
        ArmatureDataManager::getInstance()->addArmatureFileInfo("Choose-2.ExportJson");
        
        auto pos = Vec2(Director::getInstance()->getWinSize()/2) + Vec2(12, 0);
        
        auto butterfly = quickGetAnimation(0, "Choose-2", pos);
        _root->addChild(butterfly);
        

        setPackIndex(index, count);
        
        auto center = (Layout*)(Helper::seekWidgetByName(_root, "center"));
        auto snow = ParticleSystemQuad::create("choose-2/Snow.plist");
        snow->setPosition(Vec2(0, Director::getInstance()->getWinSize().height/2));
        snow->setPosVar(Vec2(Director::getInstance()->getWinSize().width / 2, 0));
        center->addChild(snow);
