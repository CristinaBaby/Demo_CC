...
bool HomeScene::init(){
    ...
    if(Scene::init())
    {
        ...
        quickLayout("monkey", _root, CC_CALLBACK_2(HomeScene::onButton, this));
        
        ArmatureDataManager::getInstance()->removeArmatureFileInfo("Choose-2.ExportJson");
        ArmatureDataManager::getInstance()->addArmatureFileInfo("Home.ExportJson");
        auto pos = Vec2(Director::getInstance()->getWinSize()/2) + Vec2(8, -15);
        
        //animation
        auto sun = quickGetAnimation(0, "Home", pos);
        _root->addChild(sun, 2);
        
        auto center = (Layout*)(Helper::seekWidgetByName(_root, "center"));
        auto girl = quickGetAnimation(1, "Home", Vec2(0,0));
        center->addChild(girl, 0);
//
