...
bool PackManager::checkHasSpecialPack()
{
    ...
    bool hasUnlockPatternPen = UserDefault::getInstance()->getBoolForKey(UserDefaultKey_PatternPenHasUnlocked, IAP_DEFAULT);
...
#error "Write platform code here!"
        ...
        if (purchaseIdentifier.size() != 0)
        {
            ...
            bool hasPurchase = UserDefault::getInstance()->getBoolForKey(userDefault.c_str(), IAP_DEFAULT);
            if (!hasPurchase) {
                return true;
            }
        }
...
void PackManager::unlockSpecialPack()
{
    for (auto iterator = _allPacks.begin(); iterator != _allPacks.end(); ++iterator)
    {
        ...
        if (iapIdentifier.size() > 0)
        {
            ...
            bool hasPurchased = UserDefault::getInstance()->getBoolForKey(userDefault.c_str(), IAP_DEFAULT);
            if (!hasPurchased)
            {
