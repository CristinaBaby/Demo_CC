...
void PackPage::createLocalPackUI(PackInfo* pack, const int index)
{
    ...
    if (purchaseIdentifier.size() > 0) {
        ...
        bool hasPurchased = UserDefault::getInstance()->getBoolForKey(userDefaultKey.c_str(), IAP_DEFAULT);
...
void PackPage::createSpecialPackUI(PackInfo* pack, const int index)
{
    ...
    //29+ icon
    Sprite *icon = Sprite::create("ui03_label_new.png");
    CCASSERT(icon, "Create pack 6+ icon failed!");
    icon->setPosition(Vec2(logo->getContentSize().width/2-10, logo->getContentSize().height/2+65));
    logo->addChild(icon);
    
    //unlock logo
    auto unlock = Sprite::create("ui03_btn_download.png");
    ...
    if (purchaseIdentifier.size() > 0)
    {
        ...
        if (!hasPurchased)
        {
            if (!_ispurchasing)
            {
                ...
                auto runingLayer = dynamic_cast<Layer*>(Director::getInstance()->getRunningScene()->getChildren().at(0));
                if (runingLayer) {
                    layer->showInLayer(runingLayer);
                }
#else
                //if has not been purchased, purchase it
                _appBilling.purchase(purchaseIdentifier.c_str());
