...
void PageControl::setPagesCount(const int pagesCount)
{
    ...
    for (int i = 0; i < _pagesCount; ++i)
    {
        ...
        normal->setPosition(Vec2(offset+normal->getContentSize().width/2, normal->getContentSize().height/2));
...
{
    _selectedControl->setPosition(index * _selectedControl->getContentSize().width + INTERVAL * index + _selectedControl->getContentSize().width/2, _selectedControl->getPositionY());
}
