...
bool ScribbleSauce::init(Size contentSize)
{
    ...
    m_rCanvas=RenderTexture::create(m_sContentSize.width,m_sContentSize.height,Texture2D::PixelFormat::RGBA8888,
    ...
    listener->onTouchMoved=[=](Touch *touch, Event *unusedEvent){
        ...
        if (m_tBrush) {
            ...
            penSize = 3;
#endif
            vector<Point> lPositions = interpolate(touch->getPreviousLocation(),touch->getLocation(),penSize);
    ...
    listener->onTouchCancelled=[=](Touch *touch, Event *unusedEvent){
        if(callBack != nullptr)
...
void ScribbleSauce::paint(RenderTexture *pCanvas, vector<Point> pWorldPositions,Texture2D* texture)
{
    ...
    for (int i = 0; i < pWorldPositions.size(); ++i) {
        ...
        log("=lPosition===%f,%f",lPosition.x,lPosition.y);
...
void ScribbleSauce::paint(Point pStartPosition, Point pEndPosition){
    int penSize = 1;
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
    penSize = 1;
#else
    penSize = 3;
#endif
    vector<Point> lPositions = interpolate(pStartPosition, pEndPosition, penSize);
    this->paint(m_rCanvas, lPositions,m_tBrush);
}
