...
#include "NachoEatScene.h"
#include "SceneManager.h"

NachoEatScene::NachoEatScene()
{
    m_nRequestType = -1;
    m_nEatCount = 0;
}
...
bool NachoEatScene::init()
{
    ...
    //    bg
    m_pBg = Sprite::create(localRootPath("bg/nachos_bg5.jpg"));
...
//        m_pFood->addChild(pNachos);
    ...
    AudioHelp::getInstance()->playEffect("vo_nacho_eat.mp3");
    this->runAction(Sequence::create(DelayTime::create(2),
                                     CallFunc::create([=](){
        AudioHelp::getInstance()->playEffect("vo_nacho_eat1.mp3");
    }), NULL));
...
void NachoEatScene::onEnter()
{
    ExtensionScene::onEnter();
    Analytics::getInstance()->sendScreenEvent(Flurry_EVENT_EAT);
}

void NachoEatScene::onShopItemBuy(cocos2d::Ref *pRef)
{
}

void NachoEatScene::dragNodeTouchBegin(DragNode* pDragNode,Point worldPoint)
{
    pDragNode->setLocalZOrder(20);
}

void NachoEatScene::dragNodeTouchMoved(DragNode* pDragNode,Point worldPoint)
{
    ...
    if (name=="spoon"){
        ...
        if (rect.containsPoint(worldPoint)) {
            ...
            pDragNode->runAction(Sequence::create(RotateBy::create(0.5, 20),
                                                  ...
                                                  CallFunc::create([=](){
                pDragNode->setLocalZOrder(20);
                m_pNachos->doEat(pDragNode->getWorldSpaceActionPoint());
                
                Sprite* pNachos = Sprite::create(FileUtils::getInstance()->getWritablePath()+"nachos_dec.png");
                Sprite* pMask = Sprite::create("content/eatShape2.png");
                Size size = pMask->getContentSize();
                if(pNachos){
                    Size size2 = pMask->getContentSize();
                    pNachos->setPosition(Vec2(size.width*0.5-size2.width/count-30+110*(m_nEatCount/count), size.height*0.5-size2.height/count-20+70*(m_nEatCount%count)));
                }
...
void NachoEatScene::dragNodeTouchEnded(DragNode* pDragNode,Point worldPoint){
    
}

void NachoEatScene::dragNodeClicked(DragNode* pDragNode,Point worldPoint)
{
    
}

void NachoEatScene::onButtonCallback(Button* btn)
{
    int tag = btn->getTag();
...
void NachoEatScene::onPermissionGrantedResult(int requestCode,bool bGranted){
    if (requestCode == 1) {
        if (bGranted) {
            //add your code....
...
void NachoEatScene::_initData()
{
    setExPath("content/make/nachos/1/");
}

void NachoEatScene::_showNachos()
{
    ...
    if (pNachos) {
        size = pNachos->getContentSize();
    }
    m_pNachos = MakeEatLayer::create();
    
    MyRenderTexture* rt = MyRenderTexture::create(size.width,size.height);
    rt->setPosition(Vec2(0, 0));
    rt->beginWithClear(0, 0, 0, 0);
    if (pNachos) {
        pNachos->setPosition(Vec2(size.width/2, size.height/2));
        pNachos->visit();
    }
    rt->end();
    Director::getInstance()->getRenderer()->render();
    ...
    m_pNachos->setRenderTexture(rt);
    m_pNachos->setPosition(Vec2(-size.width/2, -size.height/2));
    m_pNachos->setShapeTexture(Sprite::create("content/eatShape2.png")->getTexture());
    this->addChildToContentLayer(m_pNachos);
    CMVisibleRect::setPositionAdapted(m_pNachos, 320-size.width/2-visibleSize.width, 350-size.height/2+60);
    m_pNachos->runAction(MoveBy::create(1, Vec2(visibleSize.width, 0)));
    m_pNachos->setLocalZOrder(5);
}

void NachoEatScene::_showSpoon()
{
...
void NachoEatScene::_reset()
{
    ...
    m_pNachos->runAction(Sequence::create(MoveBy::create(1, Vec2(visibleSize.width, 0)),
                                         CallFunc::create([=](){
        m_pNachos->removeFromParent();
    }), NULL));
    m_pSpoon->runAction(Sequence::create(MoveBy::create(1, Vec2(visibleSize.width, 0)),
                                         CallFunc::create([=](){
...
void NachoEatScene::_showEatScene()
{
    ...
    m_pEatLayer->onEatBeginCallback = CC_CALLBACK_2(NachoEatScene::onEatBeinCallback, this);
    m_pEatLayer->onEatEndCallback = CC_CALLBACK_1(NachoEatScene::onEatEndCallback, this);
...
RenderTexture* NachoEatScene::getResultRender()
{
...
void NachoEatScene::onEatBeinCallback(Vec2 location,bool drinkable)
{
    
}

void NachoEatScene::onEatEndCallback(Vec2 location)
{
    if (m_pTip) {
...
void NachoEatScene::onKeyBackClicked()
{
    ...
    log("========NachoEatScene::onKeyBackClicked");
...
void NachoEatScene::onBgCallback(int index)
{
...
void NachoEatScene::_sendEmail()
{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
    Image* pImage = getResultRender()->newImage();
    pImage->saveToFile(SSCFileUtility::getStoragePath()+"shotscreen.png", false);
    pImage->autorelease();
    SSCInternalLibManager::getInstance()->sendEmailAndFilePic("Nachos!",
                                                              "<p>Look at the Nachos I just made.Isn't it so lovely? There are still tons of tools and decorations in Nachos you'll want to try!Stop hesitating! Download now!You are gonna love it!</p>  <p><a href=‘http://itunes.apple.com/app/id1188565450’>http://itunes.apple.com/app/id1188565450</a></p>",
    ...
    pImage->autorelease();
    SSCInternalLibManager::getInstance()->sendEmailAndFilePic("Nachos!",
                                                              "Look at the Nachos I just made.Isn't it so lovely? There are still tons of tools and decorations in Nachos you'll want to try!Stop hesitating! Download now!You are gonna love it!",
                                                              (SSCFileUtility::getStoragePath()+"/shotscreen.png").c_str());
