...
void DragNode::ccTouchMoved(cocos2d::Touch *pTouch, cocos2d::Event *pEvent)
{
    ...
    if (m_cLimitRect.equals(Rect::ZERO)) {
        ...
        Point actionPosNew  = Vec2(-this->getWorldSpaceActionPoint(), pTouch->getDelta());
        Rect rect = m_cLimitRect;
        rect.origin = m_cLimitRect.origin+m_pSprite->getAnchorPointInPoints()-m_cActionPoint;
        if(rect.containsPoint(newPtWold)){
            this->setPosition(newPt);
            m_cOffset = offset;
            if (dragNodeTouchMoved && !ignoreMoving) {
                dragNodeTouchMoved(this, getWorldSpaceActionPoint());
            ...
            Vec2 pos = newPtWold;
            if (newPtWold.x<rect.getMinX()) {
                pos.x = rect.getMinX();
            }
            if (newPtWold.x>rect.getMaxX()) {
                pos.x = rect.getMaxX();
            }
            if (newPtWold.y<rect.getMinY()) {
                pos.y = rect.getMinY();
            }
            if (newPtWold.y>rect.getMaxY()) {
                pos.y = rect.getMaxY();
            }
            //            m_cOffset = pos-this->getPosition();
            this->setPosition(pos);
            if (dragNodeTouchMoved && !ignoreMoving) {
                dragNodeTouchMoved(this, getWorldSpaceActionPoint());
            }
        }
        //        m_cOffset.x = MIN(fabsf(m_cOffset.x), m_cLimitRect.size.width);
        //        m_cOffset.y = MIN(fabsf(m_cOffset.y), m_cLimitRect.size.height);
    }
    m_cOffset = offset;
}
