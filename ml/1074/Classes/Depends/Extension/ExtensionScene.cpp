...
void ExtensionScene::onEnterTransitionDidFinish()
{
...
//    int tag = btn->getTag();
    ...
    if (GameUILayoutLayer::eUIButtonTagMenu==btn->getTag()){
        AudioHelp::getInstance()->playResetEffect();
        MenuLayer* pMenuLayer = MenuLayer::create();
        pMenuLayer->showMenu(GameDataManager::getInstance()->m_nPackage,true);
