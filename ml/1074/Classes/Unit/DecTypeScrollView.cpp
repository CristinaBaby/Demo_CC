...
void DecTypeScrollView::unlock()
{
    for (auto it = m_DecTypeVector.begin(); it!=m_DecTypeVector.end(); ++it) {
        ConfigManager::getInstance()->setVidioUnlocked(it->decTypeName+"_type", 0);
    }
    ...
    for (auto it = vec.begin(); it!=vec.end(); ++it) {
        ...
        if (i == count-lockCount && !IAPManager::getInstance()->getItemIsBought(1)) {
            if (!ConfigManager::getInstance()->getVideoUnLocked(it->decTypeName+"_type", 0)) {
                ...
                pLock->setName(LOCK_NAME_REWARD);
                pLock->setPosition(Vec2(130, 80));
                
                pBtn->setUserData(pLock);
                LockUIManager::getInstance()->registerLock(it->decTypeName+"_type", 0, pBtn);
                LockUIManager::getInstance()->setLockOriginPos(Vec2(130, 80));
            }
        }
...
void DecTypeScrollView::selectBtn(int index)
{
    ...
    if (m_bSingleAsset) {
        btn->loadTextures(btnPathEX+data.decIconName+"_h.png",btnPathEX+data.decIconName+"_h.png",btnPathEX+data.decIconName+"_h.png");
    }else{
        btn->loadTextures(btnPathEX+"dec_icon_h.png",btnPathEX+"dec_icon_h.png",btnPathEX+"dec_icon_h.png");
    }
...
//                    m_pCurTypeBtn->setPosition(Vec2(m_pCurTypeBtn->getPositionX(), btn->getPositionY()));
                ...
                DecorateTypeConfigData data = m_DecTypeVector.at(tag);
                if (m_bSingleAsset) {
                    m_pCurTypeBtn->loadTextures(btnPathEX+data.decIconName+".png",btnPathEX+data.decIconName+".png",btnPathEX+data.decIconName+".png");
                }else{
                    m_pCurTypeBtn->loadTextures(btnPathEX+"box_1_1.png",btnPathEX+"box_1_1.png",btnPathEX+"box_1_1.png");
                }
            ...
            if (btn) {
                if (m_bSingleAsset) {
                    ...
                    btn->loadTextures(btnPathEX+"box_1_2.png",btnPathEX+"box_1_2.png",btnPathEX+"box_1_2.png");
                }
            }
            if (m_bSelectedAnimate) {
...
//                                                                                ScaleTo::create(1, 1), NULL)));
                ...
                btn->runAction(MoveBy::create(0.5, Vec2(0, 20)));
            }
            m_pCurTypeBtn = btn;
            if(onItemCellSelected){
