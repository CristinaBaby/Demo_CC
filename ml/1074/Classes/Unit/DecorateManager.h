...
public:
    ...
    void setEatableLayer(MakeEatLayer*layer){
        m_pEatableLayer = layer;
    ...
    void showFinishedFood(Node* pParent){
        ...
        if (m_pFoodUneatableFrontLayer){
            Sprite* pNode = (Sprite*)m_pFoodUneatableFrontLayer->getChildByName("spoon");
            if (pNode) {
    ...
    void saveFoodImage();
    void setShapeTexture(std::string path){
        shapTexture = path;
        if (m_pEatableLayer) {
            m_pEatableLayer->setShapeTexture(Sprite::create(path)->getTexture());
        }
        if (m_pEatableFrontLayer) {
            m_pEatableFrontLayer->setShapeTexture(Sprite::create(path)->getTexture());
        }};
    bool getEatableLayerOpacity(Vec2 pos);
    int  m_nEatPercent;
protected:
    std::string shapTexture;
    void _initFoodLayer();
