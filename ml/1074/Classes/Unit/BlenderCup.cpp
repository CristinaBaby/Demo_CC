...
bool BlenderCup::init(bool physics)
{
    ...
    m_pShadow = Sprite::create("content/make/blender/blender_cup_shadow.png");
    this->addChild(m_pShadow);
    m_pShadow->setPosition(pCocosNode->getChildByName("blender_cup_shadow")->getPosition());
    
    m_pCupBack = Sprite::create("content/make/blender/blender_cup_down.png");
    this->addChild(m_pCupBack);
    m_pCupBack->setPosition(pCocosNode->getChildByName("blender_cup_shadow")->getPosition());
    
    m_pCupFront = Sprite::create("content/make/blender/blender_cup_on.png");
    ...
    Sprite*pNode = Sprite::create("content/make/blender/blender_cup_mask.png");
    ...
    m_pWaterDrop = Sprite::create("content/make/blender/blender_cup_water.png");
    this->addChild(m_pWaterDrop,11);
    m_pWaterDrop->setPosition(pCocosNode->getChildByName("blender_cup_water")->getPosition());
    
    m_pLid = Sprite::create("content/make/blender/blender1_lid.png");
    ...
    if(m_bPhysics){
        auto body2 = PhysicsBody::createBox(m_pLid->getContentSize());
        body2->setDynamic(false);
        m_pLid->setPhysicsBody(body2);
    }
...
void BlenderCup::initWithIndex(int flavor)
{
    ...
    ostr<<"content/make/blender/cup_juice_down/"<<flavor+1<<".png";
    ...
    ostr2<<"content/make/blender/cup_juice_on/"<<flavor+1<<".png";
...
void BlenderCup::addIces(int iceType,int count)
{
    ...
    for (int i = 0; i<count; i++) {
        std::stringstream ostr;
        ostr<<"content/make/ice/"<<iceType+1<<".png";
...
void BlenderCup::crushIceManual(int step)
{
    ...
    if(step==0) {
        ...
        for_each(m_IcesVector.begin(),m_IcesVector.end(),[=,&tempVector](Sprite*pIce){
            std::stringstream ostr;
            ostr<<"content/make/ice/"<<m_nIceType+1;
        ...
        m_pWaterFront->runAction(Sequence::create(ScaleTo::create(1, 1,1.1),
            ...
            std::stringstream ostr;
            ostr<<"content/make/blender/juice_animation/"<<m_nFlavorIndex+1<<"_";
            m_pWater->setScale(1);
...
void BlenderCup::showLid(bool show,bool animate)
{
    ...
    if (show) {
        ...
        if (animate) {
            m_pLid->runAction(Sequence::create(MoveBy::create(1, Vec2(-300, -1000)), NULL));
        }else{
        }
    }else{
        if (animate) {
...
//        pParticle->setName("bubble");
    ...
    setWaterPercent(90);
    std::stringstream ostr;
    ostr<<"content/make/blender/juice_animation/"<<m_nFlavorIndex+1<<"_3.png";
...
void BlenderCup::updateWater(int index)
{
    ...
    std::stringstream ostr;
    ostr<<"content/make/blender/cup_juice_down/"<<index+1<<".png";
    if (m_pWater) {
...
void BlenderCup::addWater(float height)
{
...
void BlenderCup::resumeAddWater()
{
...
//    pParticle->setName("juice_stream");
    ...
    ostr<<"content/make/machine/juice_animation/"<<m_nFlavorIndex+1<<"_";
