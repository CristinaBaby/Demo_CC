...
//            }
            ...
            bool locked = false;
            if (!IAPManager::getInstance()->getItemIsBought(m_nIAPPack)){
                if ((index*colNum+i)%2==1){
    ...
    if (m_pScrollView->getDirection()==ui::ScrollView::Direction::HORIZONTAL) {
        ...
        m_pBoard->setPosition(Vec2( getContentSize().width*0.5, getContentSize().height*0.5-10));
    }else{
