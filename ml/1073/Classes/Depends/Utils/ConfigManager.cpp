...
void ConfigManager::loadFile(std::string path)
{
    ...
    });
    {
        ...
        auto itDec = m_Map.find("decoration");
        ValueMap tempMap;
        if (itDec!=m_Map.end()) {
            ValueMap map = itDec->second.asValueMap();
            for (auto obj:map) {
                std::string type = obj.first;
                GameDataManager::getInstance()->updateVideoLockedString(type);
            }
        }
    }
}
...
bool ConfigManager::getVideoUnLocked(std::string type,int index)
{
    if(type=="pack"){
        ...
        return UserDefault::getInstance()->getBoolForKey(("pack"+std::to_string(index)).c_str(),false);
    }else{
//        if (IAPManager::getInstance()->getItemIsBought(1)) {
//            return true;
//        }
...
void ConfigManager::setVidioUnlocked(std::string type,int index)
{
    ...
    GameDataManager::getInstance()->setVideoUnlocked(type+"_type", index);
    if(type=="pack"){
