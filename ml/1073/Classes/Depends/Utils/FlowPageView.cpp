...
bool FlowPageView::scrollPages(Vec2 touchOffset)
{
//    PageView::scrollPages(touchOffset);
    ...
    switch (_touchMoveDirection)
    {
        case TouchDirection::LEFT: // left
            ...
            if (_rightBoundaryChild->getRightBoundary()+(getContentSize().width-m_PageSize.width)/2 + touchOffset.x <= _rightBoundary)
            {
                realOffset.x = _rightBoundary - _rightBoundaryChild->getRightBoundary()-(getContentSize().width-m_PageSize.width)/2;
        ...
        case TouchDirection::RIGHT: // right
            ...
            if (_leftBoundaryChild->getLeftBoundary()-(getContentSize().width-m_PageSize.width)/2 + touchOffset.x >= _leftBoundary)
            {
                realOffset.x = _leftBoundary - _leftBoundaryChild->getLeftBoundary()+(getContentSize().width-m_PageSize.width)/2;
                movePages(realOffset);
                return false;
            }
            break;
