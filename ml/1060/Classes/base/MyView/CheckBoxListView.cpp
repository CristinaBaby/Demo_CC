...
void CheckBoxListView::chooseIndex(int _index){
    if(_index <0||_container == nullptr || _index>=_container->getChildren().size()|| _curent == _container->getChildren().at(_index))
        return;
    if(_curent != nullptr)
        _curent->setSelected(false);
