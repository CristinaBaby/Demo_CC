...
void MyScollView::setAdapter(MyBaseAdapter *base){
    ...
    switch (getDirection()) {
        case Direction::VERTICAL:
            ...
            for(int i = (int)(nodeCache.size()-1) ;i>=0;i--){
...
void MyScollView::touchEnd(EventSprite* btn,int tag, Touch *pTouch){
    if(!isMove||(btn->isRequsetMove())){
        do {
            ...
            std::function<void(EventSprite*,int,MyScollView*)> cuCLick = currAdpter->getItemClick();
            CC_BREAK_IF(!cuCLick);
            cuCLick(btn,(int)nodeCache.getIndex(btn) ,this);
        }while (false);
    } else
