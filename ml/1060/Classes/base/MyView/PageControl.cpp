...
void PageControl::showPage(EventSprite* page,bool flag) {
    ...
    if(flag) {
        ...
        runAction(Sequence::create(DelayTime::create(0.15f), CallFunc::create([=]{
           // scaleNode();
...
void PageControl::showPage(int i,bool flag){
    if(i<0||i>=nodeCache.size())
        return;
    showPage(nodeCache.at(i),flag);
}

void PageControl::showPage(int i,float time){
    if(i<0||i>=nodeCache.size())
        return;
    stopAllActions();
