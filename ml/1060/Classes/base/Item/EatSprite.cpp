...
void EatSprite::CloneSprite(){
 ...
 void EatSprite::addDounSprite(Sprite* s){
     if(s != nullptr){
         Color3B _color;
         _color.r = s->getColor().r*.8;
         _color.g = s->getColor().g*.8;
...
void  EatSprite::soundUpdate(float){
    isPlayA = false;
}
...
{
//        touchCount++;
//    if(!isPlayA&&touchCount%6 == 0){
//        isPlayA = true;
//        this->unschedule(schedule_selector(EatSprite::soundUpdate));
//        scheduleOnce(schedule_selector(EatSprite::soundUpdate), 0.5);
//        SimpleAudioEngine::getInstance()->playEffect("sound/Sound/ahaa.mp3");
//    }
//    if(!isPlayA){
...
//获取不透明像素点
long EatSprite::calcute(Image *pImage)
{
    long lCounter = 0;
