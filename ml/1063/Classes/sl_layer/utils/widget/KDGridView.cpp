...
void KDGridView::moveNextStep()
{
    if (!m_bLock)
    {
        ...
        FiniteTimeAction*  action = Sequence::create(DelayTime::create(0.05f), CallFunc::create(CC_CALLBACK_0(KDGridView::unlock,this)), NULL);
        runAction(action);
...
void KDGridView::adjustScrollView(float offset)
{
    ...
    FiniteTimeAction*  action = Sequence::create(DelayTime::create(0.25), CallFunc::create(CC_CALLBACK_0(KDGridView::unlock, this)), NULL);
    runAction(action);
}
