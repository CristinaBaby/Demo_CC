...
{
    if ( !Sprite::initWithFile("cocostudio/ui/dec/zoombox/box.png"))
    {
        return false;
    }
    ...
    _close = SpriteButton::create("cocostudio/ui/dec/zoombox/x.png",std::bind(&FillMaterialModel::deleteSelf, this,std::placeholders::_1));
    _magnify = Sprite::create("cocostudio/ui/dec/zoombox/Y.png");
    _item = Sprite::create(ImageName);
    
    _close->setPosition(0, this->getContentSize().height);
    _magnify->setPosition(this->getContentSize().width, 0);
    _item->setPosition(this->getContentSize()/2);
...
bool FillMaterialModel::onTouchBegan(Touch *touch, Event *unused_event)
{
    ...
    if(!_isTipsFrameShow)
    {
        ...
        this->setSpriteFrame(Sprite::create("cocostudio/ui/dec/zoombox/box.png")->getSpriteFrame());
        _isTipsFrameShow = true;
    }
    //停止隐藏提示控件的动作
    if(this->isScheduled(schedule_selector(FillMaterialModel::updateTipsFrameStatus)))
...
void FillMaterialModel::stopTipsFrameStatus()
{
    ...
    this->setSpriteFrame(Sprite::create("cocostudio/ui/dec/zoombox/box1.png")->getSpriteFrame());
