...
void AdapterScreen::setUpScreenAdapter(float width, float height)
{
    ...
    glview->setDesignResolutionSize(frameSize.width / scale, frameSize.height / scale, ResolutionPolicy::SHOW_ALL);
    ...
    log("g_oOffset:x=%f,y=%f", g_oOffset.x, g_oOffset.y);
    Size visibleSize = director->getInstance()->getVisibleSize();
    log("visibleSize:width=%f,height=%f",  visibleSize.width, visibleSize.height);
    Point orginPoint = director->getInstance()->getVisibleOrigin();
    log("orginPoint:x=%f,y=%f", orginPoint.x, orginPoint.y);
    log("=====================================");
}
...
Vec2 AdapterScreen::getRealPosition(float x, float y, bool bTop , bool bBottom , bool bLeft
                                    , bool bRight ) {
    ...
    if (bLeft){
        ...
        x = originVec2.x + visibleSize.width - 960+x;
    } else
             x = g_oOffset.x + x;
   ...
   if (bBottom){
        ...
        y = originVec2.y + visibleSize.height - 640+y;
    } else
        y = g_oOffset.y + y;
    return Vec2(x,y);
}
