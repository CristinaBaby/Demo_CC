...
{
    auto path = FileUtility::getStoragePath() +temp + "/" + temp;
//    auto path = FileUtility::getStoragePath();
    DIR *dir;
    dirent *dir_info;
    char file_path[PATH_MAX];
    if(FileUtility::is_dir(path.c_str()))
...
std::string FilePathUtil::newCachePath(const string& path) {
    string name = path;
    
    FileUtility::createDirectory((FileUtility::getStoragePath()+ "/"+name).c_str());
    FileUtility::createDirectory((FileUtility::getStoragePath()+ "/"+name +"/"+ name).c_str());
    string rootPath;
//    rootPath = FileUtility::getStoragePath()+"/"+name + "/" + name + "/";
    rootPath = name + "/" + name + "/";
    struct timeval tv;
    gettimeofday(&tv,NULL);
