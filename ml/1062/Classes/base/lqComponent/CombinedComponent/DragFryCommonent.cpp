...
bool DragFryCommonent::init(){
    if(LQComponent::init()) {
        ...
        pMove->addListners(ComponentTouchEnd, CC_CALLBACK_3(DragFryCommonent::touchOperate,this));
        pMove->addListners(ComponentTouchCancle, CC_CALLBACK_3(DragFryCommonent::touchOperate,this));
        pMove->addListners(ComponentTouchMove, CC_CALLBACK_3(DragFryCommonent::touchOperate,this));
        addComponent(pMove);
        return true;
    }
    return false;
...
void DragFryCommonent::fryUpdate(float f){
    ...
    if(_percent<=0.75){
        int tcount = int(_percent/(0.75/(fryNodes.size()-1)));
...
void DragFryCommonent::touchOperate(Node* _node,Component* _move,OperateListner* _lis){
