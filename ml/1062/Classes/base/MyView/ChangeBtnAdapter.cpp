...
EventSprite * ChangeBtnAdapter::getItem(int position){
    auto btn = EventSprite::create(file_path.at(position));
    return btn;
}

void ChangeBtnAdapter::chaneBtnClick (EventSprite* btn,int index,MyScollView* sol){
    if(currenIndex != -1){
 ...
 void ChangeBtnAdapter::setUpItemClick(std::function<void(EventSprite*,int,MyScollView*)> upClick) {
...
void ChangeBtnAdapter::setItemClick(std::function<void(EventSprite*,int,MyScollView*)> click){
