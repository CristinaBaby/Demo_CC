...
public:
    ...
    std::string localRootPath(const std::string&  name){
        std::string file_path = "";
        size_t pos = m_sExPath.find_last_of("/");
        if (pos != std::string::npos)
        {
            file_path = m_sExPath.substr(0, pos);
        }
        pos = file_path.find_last_of("/");
        if (pos != std::string::npos)
        {
            file_path = m_sExPath.substr(0, pos+1);
        }
        return file_path+std::string(name);
    }
