...
void GameUILayoutLayer::showHomeUILayout(bool animate)
{
    ...
    if (!m_pHomeStart) {
        m_pHomeStart = _createButton("content/start/btn_start1.png");
    ...
    if (!animate) {
        ...
        CMVisibleRect::setPositionAdapted(m_pHomeStart, 320, 300);
    }else{
                                                 ...
                                                 CallFunc::create([=]()
                                                                  {
                                                                      ...
                                                                      m_pHomeStart->runAction(RepeatForever::create(Sequence::create(DelayTime::create(3),
                                                                                                                                     JumpBy::create(0.5, Vec2::ZERO, 10, 2), NULL)));
                                                                  }),
                                                 NULL));
    }
}
...
void GameUILayoutLayer::showResetLayout(bool animate,bool up)
{
    ...
    if (!animate) {
        CMVisibleRect::setPosition(m_pReset, 10, 120+(up?0:100),kBorderRight,up?kBorderTop:kBorderBottom);
    }else{
        CMVisibleRect::setPosition(m_pReset, 10, 120+(up?0:100),kBorderRight,up?kBorderTop:kBorderBottom);
...
void GameUILayoutLayer::showEatLayout(bool animate)
{
    ...
    if (!animate) {
        ...
        CMVisibleRect::setPositionAdapted(m_pEat, 420, 750);
...
void GameUILayoutLayer::showSaleLayout(bool animate)
{
    ...
    if (!animate) {
        ...
        CMVisibleRect::setPositionAdapted(m_pSale, 220, 750);
...
void GameUILayoutLayer::showEatAgainLayout(bool isDrink,bool animate)
{
    if (!m_pEatAgain) {
        if (isDrink) {
            m_pEatAgain = _createButton("content/common/button/btn_drinkagain.png");
        }else{
...
void GameUILayoutLayer::showServeUILayout(bool animate)
{
    ...
    CMVisibleRect::setPosition(m_pHome, 10, 15,kBorderRight,kBorderTop);
...
void GameUILayoutLayer::showNormalLayout(bool animate)
{
    ...
    CMVisibleRect::setPosition(m_pHome, 10,15,kBorderLeft,kBorderTop);
...
void GameUILayoutLayer::showCointLayout(bool animate){
    if (!m_pCointBg) {
        m_pCointBg = Sprite::create("content/common/heart_frame.png");
