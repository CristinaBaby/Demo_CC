...
void DecorateManager::addDecoration(cocos2d::Node *pUnit, int layerFlag,const std::string singleName,Vec2 position,int zOrder)
{
    ...
    switch (layerFlag) {
        ...
        case eDecorationLayerDrink:
        {
            ...
            if (!m_pDrinkRT) {
                m_pDrinkRT = RenderTexture::create(visibleSize.width, visibleSize.height,Texture2D::PixelFormat::RGBA8888,
            ...
            if (!m_pDrinkDecRT) {
                m_pDrinkDecRT = RenderTexture::create(visibleSize.width, visibleSize.height,Texture2D::PixelFormat::RGBA8888,
                                                      GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
...
bool DecorateManager::getEatableLayerOpacity(Vec2 pos)
{
    if (m_pEatableLayer) {
        int alpha = m_pEatableLayer->getAlphaValue(pos);
        if(alpha>0){
            return true;
        }
    }
...
void DecorateManager::saveFoodImage()
{
    MyRenderTexture* rt = MyRenderTexture::create(visibleSize.width, visibleSize.height,Texture2D::PixelFormat::RGBA8888,
                                                  GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
    
    rt->beginWithClear(1.0f, 1.0f, 1.0f, 0);
    if (m_pEatableLayer) {
        m_pEatableLayer->visit();
    }
    if (m_pEatableFrontLayer) {
        m_pEatableFrontLayer->visit();
    }
    rt->end();
    Director::getInstance()->getRenderer()->render();
    ...
    issuccess = pImage->saveToFile(FileUtils::getInstance()->getWritablePath()+ "eattemp.png", false);
    pImage->autorelease();
}
...
void DecorateManager::saveEatLayer()
{
    {
        //        uneatable back
        RenderTexture* pUneatableBackRender =  RenderTexture::create(visibleSize.width, visibleSize.height,Texture2D::PixelFormat::RGBA8888,
                                                                     GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
    ...
    {
        RenderTexture* pEatableBackRender =  RenderTexture::create(visibleSize.width, visibleSize.height,Texture2D::PixelFormat::RGBA8888,
                                                                   GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
...
Layer* DecorateManager::getUneatableLayer()
{
    ...
    RenderTexture* rt = RenderTexture::create(visibleSize.width*1.4, visibleSize.height,Texture2D::PixelFormat::RGBA8888,
                                              GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
    rt->setPosition(Vec2(visibleSize.width*0.7, visibleSize.height*0.5));
    rt->beginWithClear(0.5f, 0.5f, 0.5f, 0);
    if (m_pFoodUneatableBackLayer) {
        m_pFoodUneatableBackLayer->visit();
    }
    if (m_pDrinkUneatableBackLayer) {
    ...
    //    image->autorelease();
    //    pSprite->setPosition(Vec2(visibleSize.width*0.7, visibleSize.height*0.5));
...
Layer* DecorateManager::getFoodFrontLayer()
{
    ...
    rt->beginWithClear(1.0f, 1.0f, 1.0f, 0);
    if (m_pFoodUneatableFrontLayer) {
        Node* pNode = m_pFoodUneatableFrontLayer->getChildByName("bag");
        if (pNode) {
...
Layer* DecorateManager::getDrinkFrontLayer()
{
    ...
    //    rt->setPosition(visibleSize*0.5);
    rt->setPosition(Vec2(visibleSize.width*0.7, visibleSize.height*0.5));
    //    Node* pNode = m_pDrinkUneatableFrontLayer->getChildByName("straw_clipping");
    //    if (pNode) {
    //        pNode->setVisible(false);
    //    }
...
MakeEatLayer* DecorateManager::getEatableLayer()
{
    ...
    MyRenderTexture* rt = MyRenderTexture::create(visibleSize.width*1.4, visibleSize.height*1.4,Texture2D::PixelFormat::RGBA8888,
                                                  GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
    rt->setPosition(Vec2(visibleSize.width*0.7,visibleSize.height*0.7));
    rt->beginWithClear(0, 0, 0, 0);
    if (m_pFoodLayer) {
        m_pFoodLayer->visit();
    }
    ...
    MyRenderTexture* rt2 = MyRenderTexture::create(visibleSize.width*1.4, visibleSize.height*1.4,Texture2D::PixelFormat::RGBA8888,
                                                   GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
    rt2->setPosition(Vec2(visibleSize.width*0.7,visibleSize.height*0.7));
    
    Sprite* pSpriteTexture = Sprite::createWithTexture(rt->getSprite()->getTexture());
    Sprite* pSprite = Sprite::createWithTexture(rt->getSprite()->getTexture());
    //    Vec2 position = pSprite->getPosition();
    pSprite->setPosition(Vec2(visibleSize.width*0.7, visibleSize.height*0.7));
    pSprite->setFlippedY(-1);
    
    pSpriteTexture->setPosition(Vec2(visibleSize.width*0.7, visibleSize.height*0.7));
...
MakeEatLayer* DecorateManager::getEatableFrontLayer()
{
    ...
    rt->setPosition(Vec2(visibleSize.width*0.7, visibleSize.height*0.7));
    if (m_pFoodUneatableBackLayer) {
        m_pFoodUneatableBackLayer->setVisible(false);
    }
    if (m_pFoodUneatableFrontLayer) {
        m_pFoodUneatableFrontLayer->setVisible(false);
    }
    rt->beginWithClear(0, 0, 0, 0);
    if (m_pAllFrontLayer) {
    ...
    if (m_pFoodUneatableBackLayer) {
        m_pFoodUneatableBackLayer->setVisible(true);
    }
    if (m_pFoodUneatableFrontLayer) {
        m_pFoodUneatableFrontLayer->setVisible(true);
    }
    MyRenderTexture* rt2 = MyRenderTexture::create(visibleSize.width*1.4, visibleSize.height*1.4,Texture2D::PixelFormat::RGBA8888,
                                                   GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
    rt2->setPosition(Vec2(visibleSize.width*0.7, visibleSize.height*0.7));
...
{
    Layer* pLayer = Layer::create();
    RenderTexture* rt = RenderTexture::create(visibleSize.width*1.4, visibleSize.height*1.4,Texture2D::PixelFormat::RGBA8888,
                                              GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
    rt->setPosition(Vec2(visibleSize.width*0.7, visibleSize.height*0.7));
...
void DecorateManager::_updateDrink(float dt)
{
    ...
    DecorateManager::getInstance()->m_nDrinkPercent -=2;
    _showDrink(DecorateManager::getInstance()->m_nDrinkPercent);
    
    //    if(m_pStrawClipping){
    //        m_pStrawClipping->setPosition(m_pStrawClipping->getPosition()-Vec2(0,(100-DecorateManager::getInstance()->m_nDrinkPercent)*m_pStrawClipping->getStencil()->getContentSize().height/100));
    //        Node* pStraw = m_pStrawClipping->getChildByName("straw");
    //        pStraw->setPosition(pStraw->getPosition()+Vec2(0,(100-DecorateManager::getInstance()->m_nDrinkPercent)*m_pStrawClipping->getStencil()->getContentSize().height/100));
    //    }
...
void DecorateManager::_showDrink(int percent)
{
    ...
    if (!DecorateManager::getInstance()->m_pDrinkRT) {
        DecorateManager::getInstance()->m_pDrinkRT = RenderTexture::create(visibleSize.width, visibleSize.height,Texture2D::PixelFormat::RGBA8888,
                                                                           GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
...
void DecorateManager::_updateFinishedPercent()
{
    ...
    //    if ((curPercent*100>=98*percent || percent==0) && (curPercent1*100>=98*percent1 || percent1==0)&& (curPercent2>80 || curPercent2 ==0) && bDrinkFinish && bSauceFinish) {
    //        m_bFinishedEat = true;
    //    }
    if ((curPercent*100/percent>=m_nEatPercent || percent==0) && (curPercent1*100/percent1>=m_nEatPercent || percent1==0)&& (curPercent2>80 || curPercent2 ==0) && bDrinkFinish && bSauceFinish) {
        m_bFinishedEat = true;
    }
}
