...
void MakeEatLayer::doEat(Vec2 point,bool worldPos)
{
    if (m_pRenderTexture&&m_pShapeSprite) {
        m_pShapeSprite->setBlendFunc((ccBlendFunc){GL_ZERO,GL_ONE_MINUS_SRC_ALPHA});
        Vec2 pos = point;
        if (worldPos) {
            pos = m_pRenderTexture->convertToNodeSpace(point);
        }
        pos = ccpAdd(pos, Vec2(m_pRenderTexture->getSprite()->getContentSize().width*0.5, m_pRenderTexture->getSprite()->getContentSize().height*0.5));
    ...
    if (m_pRenderTextureInner&&m_pShapeSprite) {
        ...
        Vec2 pos = point;
        if (worldPos) {
            pos = m_pRenderTextureInner->convertToNodeSpace(point);
        }
        pos = ccpAdd(pos, Vec2(m_pRenderTextureInner->getSprite()->getContentSize().width*0.5, m_pRenderTextureInner->getSprite()->getContentSize().height*0.5));
        m_pShapeSprite->setPosition(pos);
