...
void DragNode::ccTouchMoved(cocos2d::Touch *pTouch, cocos2d::Event *pEvent)
{
    ...
    if (m_cLimitRect.equals(Rect::ZERO)) {
        ...
        Vec2 afterP0 = this->getPosition()+pTouch->getDelta();
        Vec2 afterP  = this->getParent()->convertToWorldSpace(newPt);
        
        if (m_cLimitRect.containsPoint( afterP)) {
            this->setPosition(afterP0);
            m_cOffset = offset;
        }else{
            Vec2 pos = afterP;
            if (afterP.x<m_cLimitRect.getMinX()) {
                pos.x = m_cLimitRect.getMinX();
            }
            if (afterP.x>m_cLimitRect.getMaxX()) {
                pos.x = m_cLimitRect.getMaxX();
            }
            if (afterP.y<m_cLimitRect.getMinY()) {
                pos.y = m_cLimitRect.getMinY();
            }
            if (afterP.y>m_cLimitRect.getMaxY()) {
                pos.y = m_cLimitRect.getMaxY();
            }
            pos = this->getParent()->convertToNodeSpace(pos);
            this->setPosition(pos);
            m_cOffset = pos-pTouch->getPreviousLocationInView();
        }
