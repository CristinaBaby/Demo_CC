...
//    GameUIEvent::getInstance()->nextSceneTag = GameUIEvent::eSceneTagChoosePackage;
    ...
    if (GameDataManager::getInstance()->m_nPackage==0){
        ...
        Button* pBack = m_pGameUI->getButton(GameUILayoutLayer::eUIButtonTagBack);
        CMVisibleRect::setPosition(pBack, 10, 110,kBorderLeft,kBorderTop);
    }
...
void ShareScene::onButtonCallback(Button* btn)
{
    ...
    if (GameUILayoutLayer::eUIButtonTagFav==tag) {
        ...
        if (GameDataManager::getInstance()->m_nPackage==0) {
            ...
            Sprite* pPlate = Sprite::create("content/make/japanese/decoration/plate.png");
            m_pDecoLayer->addChild(pPlate);
            pPlate->setPosition(visibleSize*0.5);
            //        pPlate->setScale(1, 0.8);
            
            MakeEatLayer* m_pHotdog = _createHotdogLayer();
            
            m_pDecManager = DecorateManager::getInstance();
            m_pDecManager->setEatableLayer(m_pHotdog);
            m_pDecoLayer->addChild(m_pHotdog);
        }
        ...
        btn->setEnabled(false);
        Dialog* dialog=Dialog::create(Size(451,446), (void*)&MENU_TYPE_1, Dialog::DIALOG_TYPE_NEGATIVE);
...
MakeEatLayer* ShareScene::_createHotdogLayer()
{
    
    Node* pFood = Node::create();
    pFood->setPosition(visibleSize*0.5);
    pFood->setScale(0.8);
    //        this->addChildToBGLayer(pFood);
    for (int i = 0; i<3; i++) {
        ...
        pDec->setScale(1, 0.8);
        pFood->addChild(pDec);
        pDec->setPosition(Vec2(0,110-100*(i%3)));
    }
    MakeEatLayer* m_pHotdog = MakeEatLayer::create();
    ...
    rt->setPosition(Vec2(visibleSize.width*0.5,visibleSize.height*0.5));
    rt->begin();
    pFood->visit();
    rt->end();
    
    MyRenderTexture* rt2 = MyRenderTexture::create(visibleSize.width, visibleSize.height,Texture2D::PixelFormat::RGBA8888,
                                                   GL_DEPTH24_STENCIL8_OES);//针对clippingNode 设置了深度
    rt2->setPosition(Vec2(visibleSize.width*0.5,visibleSize.height*0.5));
    ...
    pSprite->setPosition(Vec2(visibleSize.width*0.5, visibleSize.height*0.5));
    pSprite->setFlippedY(-1);
    
    pSpriteTexture->setPosition(Vec2(visibleSize.width*0.5, visibleSize.height*0.5));
    ...
    m_pHotdog->setRenderTexture(rt);
    
    
    m_pHotdog->setRenderTextureInner(rt2);
    return m_pHotdog;
}
...
void ShareScene::_savePhoto()
{
    ...
    SSCInternalLibManager::getInstance()->saveToAlbum(image, [=](bool ok){
        if (ok) {
            Dialog* dialog=Dialog::create(Size(451,446), (void*)&MENU_TYPE_1, Dialog::DIALOG_TYPE_SINGLE);
            dialog->setContentText("Your image has been saved to your Camera Roll!");
            Director::getInstance()->getRunningScene()->addChild(dialog, 9999, 9999);
            dialog->setPosition(Director::getInstance()->getVisibleOrigin());
        }else{
            Dialog* dialog=Dialog::create(Size(451*1.2,446*1.2), (void*)&MENU_TYPE_1, Dialog::DIALOG_TYPE_SINGLE);
            dialog->setContentText("This app does not have access to your photos.You can enable access in Privacy Setting.");
            Director::getInstance()->getRunningScene()->addChild(dialog, 9999, 9999);
            dialog->setPosition(Director::getInstance()->getVisibleOrigin());
        }
...
void ShareScene::_sendEmail()
{
    ...
    pImage->autorelease();
    SSCInternalLibManager::getInstance()->sendEmailAndFilePic("Hot Dog!",
                                                              "<p>Look at the Hot Dog I just made.Isn't it so lovely? There are still tons of tools and decorations in Hot Dog you'll want to try!Stop hesitating! Download now!You are gonna love it!</p>  <p><a href=‘http://itunes.apple.com/app/id1180851942’>http://itunes.apple.com/app/id1180851942</a></p>",
    ...
    pImage->autorelease();
    SSCInternalLibManager::getInstance()->sendEmailAndFilePic("Hot Dog!",
                                                              "Look at the Hot Dog I just made.Isn't it so lovely? There are still tons of tools and decorations in Hot Dog you'll want to try!Stop hesitating! Download now!You are gonna love it!",
                                                              (SSCFileUtility::getStoragePath()+"/shotscreen.png").c_str());
    
#endif
}

void ShareScene::_playEffectNice()
{
    int radom = arc4random()%100;
    if (radom>50){
        AudioHelp::getInstance()->playEffect("vo_tasty.mp3");
    }else {
