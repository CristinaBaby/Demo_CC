    ...
    for (int index = m_eDirection==Direction::eDirectionH?0:count-1; m_eDirection==Direction::eDirectionH?(index<count):(index>=0); m_eDirection==Direction::eDirectionH?(index++):(index--)) {
        for (int i = 0; i<colNum; i++) {
            ...
            if (decorationData.videoCount>0) {
                if (index*colNum+i>=decorationData.freeCountOrigin){
                    if (IAPManager::getInstance()->getItemIsBought(0)){
                        type = 0;
                    }else{
                        //iap物品解锁  但reward不解锁
                        if (decorationData.freeCount==decorationData.totalCount) {
                            type = (index*colNum+i-decorationData.freeCountOrigin)%2;
                        }else{
...
void ItemScrollView::_itemSelected(ItemCell* cell)
{
    ...
    if(onItemCellSelected){
        onItemCellSelected(cell->getTag(),cell->lockType,DecorateConfigData::copy(decorationData));
    }
}
