...
bool DecorateLogoLayer::init()
{
    ...
    m_pRootNode->runAction(EaseBounceOut::create(MoveBy::create(1, Vec2(0, -visibleSize.height + 30))));
    //    bg
    ...
    pBg2->setPosition(Vec2(visibleSize.width*0.5,visibleSize.height*0.5+40));
    ...
    m_pBtnOK->setPosition(Vec2(rect.getMaxX()-30, rect.getMaxY()-90));
    ...
    m_pBtnCancel->setPosition(Vec2(rect.origin.x+35, rect.getMaxY()-95));
    ...
    m_pLogoClipping = ClippingNode::create(Sprite::create("content/logo_dec/mask2.png"));
    m_pRootNode->addChild(m_pLogoClipping,5);
    m_pLogoClipping->setAlphaThreshold(0.5);
    m_pLogoClipping->setPosition(Vec2(visibleSize.width*0.5,visibleSize.height*0.5+40));
...
{
    Layer::onEnter();
    Analytics::getInstance()->sendScreenEvent(Flurry_EVENT_DECORATE_LOGO);
...
#else
    issuccess = pImage->saveToFile(FileUtils::getInstance()->getWritablePath()+"logo.png", false);
#endif
//    pImage->autorelease();
    return issuccess;
}
...
void DecorateLogoLayer::_onDecorationCallback(int index,int type,DecorateConfigData decData)
{
    ...
    if(m_sCurTypeStr=="pen_logo"){
        if (!m_pScribble) {
            ...
            m_pScribble->setShape("content/logo_dec/mask2.png");
