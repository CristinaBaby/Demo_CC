...
bool DecTypeScrollView::init(Size size)
{
    ...
    onItemCellSelected = nullptr;
    m_nPercent = 0;
    m_pBoard = Sprite::create("content/category/icon/bar.png");
    this->addChild(m_pBoard);
    //m_pBoard->setPosition(Vec2(m_pBoard->getContentSize().width*0.5, visibleSize.height/2));
...
void DecTypeScrollView::loadType(std::vector<DecorateTypeConfigData> vec)
{
    ...
    for (auto it = vec.begin(); it!=vec.end(); ++it) {
        ...
        if (m_bSingleAsset) {
            ...
            pBtn = Button::create(btnPathEX+"dec_icon.png",btnPathEX+"dec_icon.png");
        ...
        if (m_eDirection==Direction::eDirectionH) {
            pBtn->setPosition(Vec2(margin+20+deltX*0.5+(deltX+10+margin)*i, getContentSize().height/2));
        }else{
            pBtn->setPosition(Vec2(getContentSize().width/2,deltY*0.6+(deltY*scale)*(vec.size()-i-1)));
        }
        pBtn->setTag(i);
    ...
    if (m_eDirection==Direction::eDirectionH) {
        ...
        if (vec.size()>4) {
            m_pScrollView->setInnerContainerSize(Size(deltX,deltY*0.6+(deltY*scale)*m_DecTypeVector.size()));
        }
    ...
    if (type==Widget::TouchEventType::ENDED) {
        ...
        if (btn==m_pCurTypeBtn) {
            if (m_bSingleAsset) {
                ...
                btn->loadTextureNormal(btnPathEX+"dec_icon.png");
            }
