...
void GameUILayoutLayer::showHomeUILayout(bool animate)
{
    ...
    if (!animate) {
        ...
        CMVisibleRect::setPositionAdapted(m_pHomeStart, 480, 270);
        ...
        _showHomeButtonAnimation(m_pHomeMore,0.5,0.1);
        if (!gNoneIap) {
            _showHomeButtonAnimation(m_pHomeShop,0.5,0.1);
        }
        m_pHomeStart->setScale(0.01);
        m_pHomeStart->runAction(Sequence::create(Spawn::create(FadeIn::create(0.6),
...
void GameUILayoutLayer::showEatAgainLayout(bool isDrink,bool animate)
{
    ...
    if (!animate) {
        ...
        CMVisibleRect::setPositionAdapted(m_pEatAgain, 480, 320+visibleSize.height);
...
//    }
    ...
    if (!m_pBg) {
        m_pBg = _createButton("content/common/button/btn_bg.png");
        this->addChild(m_pBg);
    }
//
