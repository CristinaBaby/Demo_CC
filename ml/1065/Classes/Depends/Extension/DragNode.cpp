...
bool DragNode::ccTouchBegan(Touch *pTouch, Event *pEvent)
{
    ...
    Rect rect = Rect(-_contentSize.width*m_pSprite->getAnchorPoint().x, -_contentSize.height*m_pSprite->getAnchorPoint().y, _contentSize.width, _contentSize.height);
...
void DragNode::ccTouchMoved(cocos2d::Touch *pTouch, cocos2d::Event *pEvent)
{
    ...
    float delt = 10;
    if (fabs(offset.x)>delt||fabs(offset.y)>delt) {
        m_bIsMoved = true;
    }
    m_bIsMoving = false;
    if (fabs(offset.x)>=delt||fabs(offset.y)>=delt) {
