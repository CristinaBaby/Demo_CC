...
//            pBtn->setPosition(Vec2(margin+deltX*0.5+(deltX+10+margin)*i, getContentSize().height/2));
            ...
            pBtn->setPosition(Vec2(getContentSize().width/2,deltY*0.5+10+(deltY+margin)*(vec.size()-i-1)));
        }
        pBtn->setTag(i);
    ...
    if (m_eDirection==Direction::eDirectionH) {
        ...
        if (vec.size()>4) {
            m_pScrollView->setInnerContainerSize(Size(deltX,deltY*0.5+10+(deltY+margin)*m_DecTypeVector.size()));
        }
