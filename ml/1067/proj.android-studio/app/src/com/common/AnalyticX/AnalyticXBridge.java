...
public class AnalyticXBridge {
	...
	static void Bridge (String arg0, String [] arg1, String arg2) {
		...
		} else if (splitedString[0].equalsIgnoreCase("flurryLogEventWithParametersTimed")) {
			AnalyticXBridge.flurryLogEventWithParametersTimed(splitedString[1], arg1, arg2);
		}
	}
	
	static private void flurryLogEvent(String eventID) {
		Log.v("diwu", "flurryLogEvent() is called... eventID = " + eventID);
		if(AnalyticsCenter.getInstace().getDebugMode()) {
			DebugUtil.showToast((Context) LaunchActivity.getInstance(),"sendEvent(enter event:"+eventID+")");
		}
	...
	static private void flurryLogEventTimed(String eventID, String timed) {
		if(AnalyticsCenter.getInstace().getDebugMode()) {
			DebugUtil.showToast((Context) LaunchActivity.getInstance(),"sendEvent(exit event:"+eventID+")");
		}
	...
	static private void flurryLogEventWithParameters(String eventID, String [] parametersArray) {
		...
		for (int i = 0; i < parametersArray.length / 2; i++) {
			...
			if(parametersArray[2*i].length() == 0)
				continue;
