...
ItemCell* ItemCell::create(const std::string bgPath,const std::string bgHighlight,const std::string frontHighlight,const std::string btn,const std::string lock)
{
    
    auto pRet = new ItemCell();
    if (pRet && pRet->init(bgPath,bgHighlight,frontHighlight,btn,lock))
    {
        pRet->autorelease();
        return pRet;
    }
...
bool ItemCell::init(const std::string bgPath,const std::string bgHighlight,const std::string frontHighlight,const std::string btn,const std::string lock)
{
    if (!Node::init() )
    {
        return false;
...
bool ItemScrollView::init(Size size,bool useTableView)
{
    ...
    m_pBoard = Sprite::create("content/category/icon/dec_banner.png");
...
void ItemScrollView::reloadData()
{
    ...
    for (int index = m_eDirection==Direction::eDirectionH?0:count-1; m_eDirection==Direction::eDirectionH?(index<count):(index>=0); m_eDirection==Direction::eDirectionH?(index++):(index--)) {
        for (int i = 0; i<colNum; i++) {
            ...
            ItemCell* item = ItemCell::create(bgPath, bgHighlightPath,frontHighlightPath, std::string(ostr.str()),lockName);
