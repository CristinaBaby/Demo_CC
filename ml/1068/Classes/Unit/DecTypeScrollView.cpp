...
void DecTypeScrollView::loadType(std::vector<DecorateTypeConfigData> vec)
{
    ...
    for (auto it = vec.begin(); it!=vec.end(); ++it) {
        ...
        if (m_bSingleAsset) {
            ...
            pBtn = Button::create(btnPathEX+"box_1_2.png",btnPathEX+"box_1_2.png");
        ...
        if (m_eDirection==Direction::eDirectionH) {
            ...
            pBtn->setPosition(Vec2(getContentSize().width/2,margin+deltY*0.5+(deltY+10+margin)*(vec.size()-i-1)));
        }
        pBtn->setTag(i);
