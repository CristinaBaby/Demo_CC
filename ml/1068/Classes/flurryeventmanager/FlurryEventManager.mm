...
void FlurryEventManager::logCurrentModuleEnterEvent(string moduleName)
{
    if(Analytics::getInstance()->getDebugMode()) {
        ...
        [OMGToast showWithText:[NSString stringWithCString:message.c_str() encoding:NSUTF8StringEncoding] topOffset:50 duration:5];
    }
...
{
    if(Analytics::getInstance()->getDebugMode()) {
        std::string message = "sendEvent(exit event:"+moduleName+")";
        log(message.c_str(),nullptr);
    
        [OMGToast showWithText:[NSString stringWithCString:message.c_str() encoding:NSUTF8StringEncoding] duration:5];
    }
