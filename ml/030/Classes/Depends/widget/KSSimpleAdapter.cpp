...
string iapType[8]={"hotDog","mac","syrupcup","icecup","icecream","icespoon","mould","taste"};

string taskType[8] = {"hotDog","mac","syrupcup","icecup","icecream","icespoon","mould","taste"};
...
MenuItem* KSSimpleAdapter::getView(int position,MenuItem* cacheView,Node* parent)
{
    ...
    if (item)
    {
        ...
        if (!kIAPManager->isFree(iapType[m_iIapType].c_str(), position)) {
            ...
            Sprite* lock = nullptr;
            if (m_iIapType == 0) {
                lock = Sprite::create("images/select/btn_lock.png");
            }else{
                lock = Sprite::create("images/select_mac/machine_lock.png");
            }
