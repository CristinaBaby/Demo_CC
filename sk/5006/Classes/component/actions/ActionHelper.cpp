...
//
void ActionHelper::moveInToSceneBounce(Node* node, Vec2 endPosiont,ActionDirection direction, std::function<void()> actionEndCall,float duration)
{
    ...
    auto moveAction = Sequence::create(BezierTo::create(duration, config),endCallback,NULL);
...
void ActionHelper::moveOutofSceneAndRemoveSelf(Node* node, ActionDirection type, std::function<void()> actionEndCall)
{
    ...
    switch (type) {
        case ActionDirection::kActionDirectionBottom:
            endPosiont = Vec2(curentPosition.x,bottomRight.y*2-node->getContentSize().height*(1-node->getAnchorPoint().y)*2);
            break;
        case ActionDirection::kActionDirectionLeft:
            endPosiont = Vec2(topLeft.x*2 - node->getContentSize().width*(1-node->getAnchorPoint().x)*2, curentPosition.y);
            break;
        case ActionDirection::kActionDirectionRight:
            endPosiont = Vec2(bottomRight.x*2+node->getContentSize().width*node->getAnchorPoint().x*2, curentPosition.y);
            break;
        case ActionDirection::kActionDirectionTop:
            endPosiont = Vec2(curentPosition.x, topLeft.y*2 +node->getContentSize().width*(node->getAnchorPoint().y)*2);
            break;
        default:
            break;
    }
...
void ActionHelper::changeFade(Sprite* preSprite, Sprite* curentSprite,float duration)
{
    ...
    preSprite->runAction(Sequence::create(FadeOut::create(duration),
                                          CallFunc::create([=]()
                                                           {
                                        preSprite->setTexture(curentSprite->getTexture());
                                        curentSprite->removeFromParent();
                                        preSprite->setOpacity(255);
                                                           }),
                                    nullptr));
    curentSprite->runAction(FadeIn::create(duration));
}
...
ActionInterval* ActionHelper::moveToPostionByBezier(Vec2 startPos,Vec2 endPo,float duration,Vec2 deltaV)
{
    int delta = 1;
    delta = startPos.x>endPo.x?-1:1;
    delta = startPos.x == endPo.x? 0:delta;
    auto config = ccBezierConfig();
    config.endPosition = endPo;
    config.controlPoint_1 = config.controlPoint_2 = Vec2(startPos.x+deltaV.x*delta,startPos.y+deltaV.y);
