...
typedef std::function<void(Node*,Vec2,Vec2)> PosiontChangeCallBack;
...
public:
     ...
     */
    static void moveInToSceneBounce(Node* node, Vec2 endPosiont, ActionDirection direction, std::function<void()> actionEndCallCall = nullptr,float duration = 0.8f);
    ...
    static void changeFade(Sprite* preSprite, Sprite* curentSprite,float duration = 0.5f);
    ...
    static ActionInterval* moveToPostionByBezier(Vec2 startPos,Vec2 endPo,float duration,Vec2 delta = Vec2(200,250));
