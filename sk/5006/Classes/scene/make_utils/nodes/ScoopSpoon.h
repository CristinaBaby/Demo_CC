...
public:
    ...
    std::function<void()> onDragCallback = nullptr;
    ...
    void animationScoop(const std::string& content,std::function<void()> callback);
    void animationPour(std::function<void()> callback);
    void back();
    Sprite* getSpoon()
    {
        return _mainContent;
