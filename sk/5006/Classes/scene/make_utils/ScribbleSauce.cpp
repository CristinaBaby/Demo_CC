...
bool ScribbleSauce::init(Size contentSize)
{
    ...
    listener->onTouchMoved=[this](Touch *touch, Event *unusedEvent){
        ...
        if (m_tBrush) {
            ...
            vector<Point> lPositions = interpolate(touch->getPreviousLocation(),touch->getLocation(),2);
...
void ScribbleSauce::saveScribble(const string &name,function<void(RenderTexture*, const std::string&)> callback)
{
    if(m_rCanvas != nullptr)
        m_rCanvas->saveToFile(name, true,callback);
}
