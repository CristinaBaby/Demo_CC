...
void AddSprinkle::caculatePos(){
    if(getParent())
        caculatePos(getParent()->getContentSize());
}
...
bool AddSprinkle::touchBegan(Touch* touch,Event*){
    ...
    Rect limit;
    limit.origin = (getParent()->getContentSize()-parentSize)*0.5;
    limit.size.width = parentSize.width;
//    limit.size.height = getParent()->convertToNodeSpace(Director::getInstance()->getVisibleOrigin()+ Director::getInstance()->getVisibleSize()).y;
    limit.size.height = 10000;
