...
void ActionHelper::showBackInOut(Node* node, float delayTime,ShowDirection type, std::function<void()> actionEnd ,float time){
    Vec2 endPosiont = node->getPosition();
    ...
    auto moveAction = Sequence::create(DelayTime::create(delayTime),EaseElasticInOut::create(MoveTo::create(time, endPosiont),1.0),endCallback,nullptr);
...
void ActionHelper::showBezier(Node* node, Vec2 endPosiont,ShowDirection type,float delaytime, std::function<void()> actionEnd,float time){
    Vec2 pos;
    ...
    auto moveAction = Sequence::create(DelayTime::create(delaytime),BezierTo::create(time, config),endCallback,NULL);
    node->runAction(moveAction);
}
...
void ActionHelper::changeFade(Sprite* preSprite, Sprite* curentSprite) {
    if(preSprite->getTexture() == curentSprite->getTexture())
        return;
...
ActionInterval *ActionHelper::getJellyAction()
{
...
void ActionHelper::showTitle(Node* _node){
    _node->stopActionByTag(titleActionTag);
    showBackInOut(_node, 0.0, ActionHelper::ShowDirection::show_from_left,[_node](){
        auto _action = RepeatForever::create(Sequence::create(DelayTime::create(2), createShakeAction(), createShakeAction(),nullptr));
