...
{
    auto path = SSCFileUtility::getStoragePath() + "/" +temp;
    DIR *dir;
    dirent *dir_info;
    char file_path[PATH_MAX];
...
std::string FilePathUtil::newSharePath() {
    ...
    SSCFileUtility::createDirectory((SSCFileUtility::getStoragePath()+ "/"+name).c_str());
    SSCFileUtility::createDirectory((SSCFileUtility::getStoragePath()+ "/"+name +"/"+ "share").c_str());
    string rootPath;
    rootPath = SSCFileUtility::getStoragePath()+"/"+name + "/" + "share" + "/";
