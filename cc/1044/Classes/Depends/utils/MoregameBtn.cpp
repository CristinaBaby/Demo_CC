...
bool MoregameBtn::init(){
    if (Layer::init()) {
        Sprite* moreGameFrame = Sprite::create("images/home/home_btn_more.png");
        ...
        moreGameFrame->setPosition(Point(moreGameFrame->getContentSize().width/2.0-22, moreGameFrame->getContentSize().height/2.0-50));
        container->setPosition(Point(0,0));
        moreGameFrame->setScale(1.9f);
...
//    this->setPosition(Vec2(Director::getInstance()->getVisibleSize().width,Director::getInstance()->getVisibleOrigin().y) + Point(0, -20 + getBannerHeight()));
    kAdapterScreen->setExactPosition(this, 0 + this->getContentSize().width + 20,50 + getBannerHeight(),Vec2(1,0),kBorderRight,kBorderBottom);
