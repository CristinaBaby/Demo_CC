...
MenuItem* KFavoriteAdapter::getView(int position,MenuItem* cacheView,Node* parent)
{
    ...
    Sprite* bg = Utils::getUISprite("images/shop_fav/photo_frame0.png");
    float scaleX = (bg->getContentSize().width-60)/item->getContentSize().width;
    float scaleY = (bg->getContentSize().height-60)/item->getContentSize().height;
