...
KDGridView::KDGridView(int type):
...
KDGridView* KDGridView::create(const Size &contentSize,int type)
{
    KDGridView* grid = new KDGridView();
    if(grid && grid->initWithSize(contentSize,type))
    {
        grid->autorelease();
        return grid;
    }
...
bool KDGridView::initWithSize(const cocos2d::Size &contentSize,int type)
{
    bool bRet = false;
