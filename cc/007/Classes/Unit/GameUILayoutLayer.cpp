...
void GameUILayoutLayer::showHomeUILayout(bool animate)
{
    ...
    //    }
    if (!m_pHomeShop) {
        m_pHomeShop = _createButton("content/start/btn_store.png");
    ...
    if (!animate) {
        ...
        CMVisibleRect::setPosition(m_pHomePolicy, 15, 15,kBorderLeft,kBorderTop);
        CMVisibleRect::setPosition(m_pHomeRate, 15, 15,kBorderRight,kBorderTop);
        ...
        CMVisibleRect::setPositionAdapted(m_pHomeStart, 320, 300-1000);
        ...
        _showHomeButtonAnimation(m_pHomeRate,0.5,0.5);
...
//        m_pHomeStart->setScale(0.01);
        m_pHomeStart->runAction(Sequence::create(DelayTime::create(0.5),
                                                 FadeIn::create(0.2),
                                                 EaseBackInOut::create(MoveBy::create(1,Vec2(0, 1000))),
                                                 
                                                 CallFunc::create([=]() {
            m_pHomeStart->setEnabled(true);
            
            m_pHomeStart->runAction(RepeatForever::create(Sequence::create(RotateBy::create(1, 360),
                                                                           MoveBy::create(0.35, Vec2(0,10)),
                                                                           MoveBy::create(0.35, Vec2(0,-20)),
                                                                           MoveBy::create(0.35, Vec2(0,20)),
                                                                           MoveBy::create(0.35, Vec2(0,-10)),
...
void GameUILayoutLayer::showNextLayout(bool animate,bool up)
{
    if (!m_pNext) {
    ...
    if (!animate) {
        CMVisibleRect::setPosition(m_pNext, 10, 10+(up?0:100),kBorderRight,up?kBorderTop:kBorderBottom);
    }else{
        CMVisibleRect::setPosition(m_pNext, 10, 10+(up?0:100),kBorderRight,up?kBorderTop:kBorderBottom);
...
void GameUILayoutLayer::showNextLayout2(bool animate,bool up)
{
    if (!m_pNext) {
        m_pNext = _createButton("content/common/button/btn_next.png");
        this->addChild(m_pNext);
    ...
    if (!animate) {
        CMVisibleRect::setPosition(m_pNext, 10, 110+(up?0:100),kBorderRight,up?kBorderTop:kBorderBottom);
    }else{
        CMVisibleRect::setPosition(m_pNext, 10,110+(up?0:100),kBorderRight,up?kBorderTop:kBorderBottom);
        m_pNext->runAction(RepeatForever::create(Sequence::create(EaseBounceIn::create(ScaleTo::create(0.5, 1.1)),
                                                                  ScaleTo::create(0.5, 1),
...
void GameUILayoutLayer::showBackLayout(bool animate,bool up)
{
    if (!m_pBack) {
    ...
    if (!animate) {
        CMVisibleRect::setPosition(m_pBack, 10, 10+(up?0:100),kBorderLeft,up?kBorderTop:kBorderBottom);
    }else{
        CMVisibleRect::setPosition(m_pBack, 10, 10+(up?0:100),kBorderLeft,up?kBorderTop:kBorderBottom);
...
void GameUILayoutLayer::showBackLayout2(bool animate,bool up)
{
    if (!m_pBack) {
        m_pBack = _createButton("content/common/button/btn_back.png");
        this->addChild(m_pBack);
    ...
    if (!animate) {
        CMVisibleRect::setPosition(m_pBack, 10, 110+(up?0:100),kBorderLeft,up?kBorderTop:kBorderBottom);
    }else{
        CMVisibleRect::setPosition(m_pBack, 10, 110+(up?0:100),kBorderLeft,up?kBorderTop:kBorderBottom);
        m_pBack->runAction(RepeatForever::create(Sequence::create(EaseBounceIn::create(ScaleTo::create(0.5, 1.1)),
                                                                  ScaleTo::create(0.5, 1),
...
void GameUILayoutLayer::showResetLayout(bool animate,bool up)
{
    if (!m_pReset) {
    ...
    if (!animate) {
        CMVisibleRect::setPosition(m_pReset, 10, 200+(up?0:200),kBorderRight,up?kBorderTop:kBorderBottom);
    }else{
        CMVisibleRect::setPosition(m_pReset, 10, 200+(up?0:100),kBorderRight,up?kBorderTop:kBorderBottom);
...
void GameUILayoutLayer::showEatLayout(bool animate)
{
    ...
    if (!animate) {
        CMVisibleRect::setPosition(m_pEat, 10, 220,kBorderLeft,kBorderTop);
    }else{
        CMVisibleRect::setPosition(m_pEat, 10, 220,kBorderLeft,kBorderTop);
...
{
    if (!m_pSale) {
        m_pSale = _createButton("content/common/button/btn_serve.png");
    ...
    if (!animate) {
        CMVisibleRect::setPosition(m_pSale, 10, 260,kBorderRight,kBorderBottom);
    }else{
        CMVisibleRect::setPosition(m_pSale, 10, 260,kBorderRight,kBorderBottom);
...
void GameUILayoutLayer::showSaleAgainLayout(bool isDrink,bool animate)
{
    if (!m_pSaleAgain) {
        if (isDrink) {
            m_pSaleAgain = _createButton("content/common/button/btn_saleagain.png");
        }else{
            m_pSaleAgain = _createButton("content/common/button/btn_saleagain.png");
        }
        this->addChild(m_pSaleAgain);
    }
    
    m_pSaleAgain->setVisible(true);
    m_pSaleAgain->setEnabled(true);
    m_pSaleAgain->setTag(GameUILayoutLayer::eUIButtonTagEatAgain);
    
    m_pSaleAgain->stopAllActions();
    m_pSaleAgain->setRotation(0);
    
    if (!animate) {
        CMVisibleRect::setPositionAdapted(m_pSaleAgain, 480, 420);
    }else{
        m_pSaleAgain->setEnabled(false);
        CMVisibleRect::setPositionAdapted(m_pSaleAgain, 320, 500+visibleSize.height);
        m_pSaleAgain->runAction(Sequence::create(EaseBackOut::create(MoveBy::create(2, Vec2(0, -visibleSize.height))),
                                                 CallFunc::create([=]()
                                                                  {
                                                                      m_pSaleAgain->setEnabled(true);
                                                                      m_pSaleAgain->runAction(RepeatForever::create(Sequence::create(EaseBounceIn::create(ScaleTo::create(1, 1.1)),
                                                                                                                                     DelayTime::create(1),
                                                                                                                                     ScaleTo::create(1, 1),
                                                                                                                                     NULL)));
                                                                  }), NULL));
...
void GameUILayoutLayer::showShareUILayout(bool animate)
{
    ...
    CMVisibleRect::setPosition(m_pPhoto, 10, 10,kBorderRight,kBorderTop);
    //    CMVisibleRect::setPosition(m_pEmail, 10, 80,kBorderRight,kBorderTop);
//    CMVisibleRect::setPosition(m_pHome, 10, 30,kBorderRight,kBorderTop);
...
void GameUILayoutLayer::showServeUILayout(bool animate)
{
    if (!m_pPhoto) {
        m_pPhoto = _createButton("content/common/button/btn_photo.png");
        this->addChild(m_pPhoto);
    ...
    CMVisibleRect::setPosition(m_pPhoto, 10, 90,kBorderRight,kBorderTop);
    //    CMVisibleRect::setPosition(m_pEmail, 10, 210,kBorderRight,kBorderTop);
    CMVisibleRect::setPosition(m_pHome, 10, 30,kBorderRight,kBorderTop);
...
void GameUILayoutLayer::showMenuUILayout(bool animate)
{
    if (!m_pMenu) {
        m_pMenu = _createButton("content/common/button/btn_menu.png");
        this->addChild(m_pMenu);
    }
    m_pMenu->setScale(0.8);
    m_pMenu->setTag(eUIButtonTagMenu);
    CMVisibleRect::setPosition(m_pMenu, 10,120-20,kBorderLeft,kBorderTop);
...
void GameUILayoutLayer::showNormalLayout(bool animate)
{
    ...
    //    CMVisibleRect::setPosition(m_pMore, 80,10,kBorderLeft,kBorderTop);
    showSSCMore();
    showMenuUILayout();
}
...
void GameUILayoutLayer::showSSCMore()
{
    if (!m_pSSCMore) {
        ...
        CMVisibleRect::setPosition(moreGameButton, 160, 60,kBorderLeft,kBorderTop);
    }
}

void GameUILayoutLayer::showCointLayout(bool animate){
    if (!m_pCointBg) {
        m_pCointBg = Sprite::create("content/common/icon.png");
...
//        m_pCointLabel->setPosition(Vec2(125, 50));
//        m_pCointLabel->setTextColor(Color4B(250, 94, 123, 255));
//    }
...
//    m_pCointLabel->setTag(GameUILayoutLayer::eUILabelTagCoint);
    m_pCointBg->setScale(0.8);
    
    if (!animate) {
        CMVisibleRect::setPosition(m_pCointBg, 10, 15,kBorderNone,kBorderTop);
    }else{
        CMVisibleRect::setPositionAdapted(m_pCointBg, 320, 900+500,kBorderNone,kBorderTop);
        m_pCointBg->runAction(EaseBounceOut::create(MoveBy::create(1, Vec2(0, -500))));
...
//    }
    if(m_pCointBg){
        m_pCointBg->runAction(JumpBy::create(1, Vec2::ZERO, 5, 1));
        m_pCointBg->runAction(Sequence::create(ScaleTo::create(0.5, 1),
                                               EaseBackOut::create(ScaleTo::create(0.5, 0.8)), NULL));
    }
}
