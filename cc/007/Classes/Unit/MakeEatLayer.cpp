...
void MakeEatLayer::doEat(Vec2 point)
{
    ...
    if (m_pRenderTexture&&m_pShapeSprite) {
        ...
        location = m_pRenderTexture->convertToNodeSpace(point);
        location = ccpAdd(location, Vec2(m_pRenderTexture->getSprite()->getContentSize().width*0.5, m_pRenderTexture->getSprite()->getContentSize().height*0.5));
        m_pShapeSprite->setPosition(location);
    ...
    if (m_pRenderTextureInner&&m_pShapeSprite) {
        m_pShapeSprite->setBlendFunc((ccBlendFunc){GL_ZERO,GL_ONE_MINUS_SRC_ALPHA});
        location = m_pRenderTextureInner->convertToNodeSpace(point);
        location = ccpAdd(location, Vec2(m_pRenderTextureInner->getSprite()->getContentSize().width*0.5, m_pRenderTextureInner->getSprite()->getContentSize().height*0.5));
        m_pShapeSprite->setPosition(location);
