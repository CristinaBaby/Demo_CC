...
std::vector<Color4B> ConfigManager::getColor()
{
    return m_ColorArray;
}
...
{
    std::vector<std::string> keyArray = StringHelp::split(key, "_");
    ValueMap map = _getDecorationMap(keyArray.at(0));
    DecorateConfigData data;
    data.init();
    for (auto it = map.begin(); it != map.end();++it)
    {
        if (std::strcmp(it->first.c_str(), "freecount")==0) {
            data.freeCount = data.freeCountOrigin = it->second.asInt();
        ...
        }else if (std::strcmp(it->first.c_str(), "videocount")==0) {
            data.videoCount = it->second.asInt();
    ...
    if (data.pathName=="") {
        data.pathName = string(keyArray.at(0));
    }
    ...
    {
        auto it2 = m_UnlockTypeMap.find(keyArray.at(0));
...
std::vector<DecorateTypeConfigData> ConfigManager::getDecorateType(const std::string key,const std::string key2,const std::string key3)
{
    ...
    for (auto it = vct.begin(); it!=vct.end(); ++it) {
        ...
        if (itTypeName!=map.end()) {
            std::string str = itTypeName->second.asString();
            if (key2!="") {
                str.append("_"+key2);
            }
            if (key3!="") {
...
bool ConfigManager::getUnLocked(std::string type)
{
    auto it = m_UnlockTypeMap.find(type);
    if (it!=m_UnlockTypeMap.end()) {
...
bool ConfigManager::getVideoUnLocked(std::string type,int index)
{
    if(type=="pack"){
        if (IAPManager::getInstance()->getItemIsBought(2)) {
            return true;
        }
        if(UserDefault::getInstance()->getBoolForKey(("pack"+std::to_string(index)).c_str(),false)){
            return true;
        }
    }
    auto it = m_UnlockTypeMap.find(type);
...
void ConfigManager::setVidioUnlocked(std::string type,int index)
{
    GameDataManager::getInstance()->setVideoUnlocked(type, index);
    if(type=="pack"){
        UserDefault::getInstance()->setBoolForKey(("pack"+std::to_string(index)).c_str(),true);
    }
}