...
//    log("++++++++%f,%f",offset.x,offset.y);
    ...
    if (m_cLimitRect.equals(Rect::ZERO)) {
        ...
        if(m_cLimitRect.containsPoint(newPtWold)){
            this->setPosition(newPt);
            m_cOffset = offset;
            if (dragNodeTouchMoved && !ignoreMoving) {
                dragNodeTouchMoved(this, getWorldSpaceActionPoint());
            ...
            Vec2 pos = newPtWold;
            if (newPtWold.x<m_cLimitRect.getMinX()) {
                pos.x = m_cLimitRect.getMinX();
            }
            if (newPtWold.x>m_cLimitRect.getMaxX()) {
                pos.x = m_cLimitRect.getMaxX();
            }
            if (newPtWold.y<m_cLimitRect.getMinY()) {
                pos.y = m_cLimitRect.getMinY();
            }
            if (newPtWold.y>m_cLimitRect.getMaxY()) {
