...
MyRenderTexture * MyRenderTexture::create(int w ,int h, Texture2D::PixelFormat eFormat, GLuint uDepthStencilFormat)
{
    MyRenderTexture *pRet = new MyRenderTexture();
...
float MyRenderTexture::getAlphaWithLocation(Point point)
{
    ...
    //    log("========%hhu,%hhu,%hhu,%hhu",buffer[(y * 4 * tx + ((x * 4)+0))],buffer[(y * 4 * tx + ((x * 4)+1))],buffer[(y * 4 * tx + ((x * 4)+2))],buffer[(y * 4 * tx + ((x * 4)+3))]);
    free(buffer);
    return alpha;
}